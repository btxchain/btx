...
int GuiMain(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg.startsWith(BTX_IPC_PREFIX, Qt::CaseInsensitive) ||
            arg.startsWith(QLatin1String("btx://"), Qt::CaseInsensitive)) {
            invalid_token &= false;
    ...
    /// 6-7. Parse btx.conf, determine network, switch to network specific
    ...
    // btx: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
