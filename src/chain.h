...
public:
    ...
    //! arrived and its MatMul product-committed proof has verified (i.e. the
    //! index reached BLOCK_VALID_TRANSACTIONS and is not BLOCK_FAILED). A forged
    ...
    // Legacy members retained for compatibility with transitional code paths.
    uint32_t nNonce{0};
    uint256 mix_hash{};
    ...
    explicit CBlockIndex(const CBlockHeader& block)
        : nVersion{block.nVersion},
          ...
          matmul_digest{block.matmul_digest},
 ...
 *  been received and its MatMul product-committed proof verified -- i.e. the
 *  index reached BLOCK_VALID_TRANSACTIONS and carries no BLOCK_FAILED bit. A
 *  self-declared `matmul_digest` header with no valid body is never authenticated.
 *
...
public:
    ...
    SERIALIZE_METHODS(CDiskBlockIndex, obj)
    {
        ...
        READWRITE(obj.matmul_digest);
        READWRITE(obj.matmul_dim);
        READWRITE(obj.seed_a);
        READWRITE(obj.seed_b);

        // Legacy disk fields (kept to avoid data-loss across transition).
...
public:
    ...
    CBlockIndex* operator[](int nHeight) const
    {
        ...
        if (height >= vChain.size()) return nullptr;
    ...
    {
        if (!Contains(pindex) || pindex->nHeight == std::numeric_limits<int>::max()) {
            return nullptr;
        }
        const size_t next_height{static_cast<size_t>(pindex->nHeight) + 1};
        if (next_height >= vChain.size()) return nullptr;
        return vChain[next_height];
    }

    /** Return the maximal height in the chain. Is equal to chain.Tip() ? chain.Tip()->nHeight : -1. */
    int Height() const
    {
        if (vChain.empty()) return -1;
        const size_t max_height{vChain.size() - 1};
        if (max_height > static_cast<size_t>(std::numeric_limits<int>::max())) {
            return std::numeric_limits<int>::max();
        }
        return static_cast<int>(max_height);
    }
