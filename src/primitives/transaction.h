...
struct TransactionSerParams {
    bool allow_witness{true};
    bool allow_shielded{true};
    SER_PARAMS_OPFUNC
};
static constexpr TransactionSerParams TX_WITH_WITNESS{.allow_witness = true, .allow_shielded = true};
static constexpr TransactionSerParams TX_NO_WITNESS{.allow_witness = false, .allow_shielded = false};
static constexpr TransactionSerParams TX_NO_WITNESS_WITH_SHIELDED{.allow_witness = false, .allow_shielded = true};
...
void UnserializeTransaction(TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    const bool fAllowShielded = params.allow_shielded;
    const bool fAllowOptionalData = fAllowWitness || fAllowShielded;
    ...
    s >> tx.vin;
    if (tx.vin.size() == 0 && fAllowOptionalData) {
...
void SerializeTransaction(const TxType& tx, Stream& s, const TransactionSerParams& params)
{
    ...
    if (fAllowShielded && tx.HasShieldedBundle()) {
        flags |= 2;
    }
    if (flags) {
...
public:
    ...
    const CShieldedBundle shielded_bundle;
...
private:
    ...
    const bool m_has_shielded_bundle;
    const Txid hash;
    ...
    bool ComputeHasShieldedBundle() const;
...
public:
    ...
    bool HasShieldedBundle() const { return m_has_shielded_bundle; }
    const CShieldedBundle& GetShieldedBundle() const LIFETIMEBOUND { return shielded_bundle; }
};
...
struct CMutableTransaction
{
    ...
    bool HasShieldedBundle() const
    {
        return !shielded_bundle.IsEmpty();
    }

    const CShieldedBundle& GetShieldedBundle() const LIFETIMEBOUND
    {
        return shielded_bundle;
    }
};
