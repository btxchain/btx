...
{
    LOCK(m_banned_mutex);
    if (!m_scheduler || m_sweep_started) return;
...
void BanMan::ScheduleNextSweep()
{
    ...
    if (!m_scheduler || m_banned.empty()) return;

    int64_t earliest = std::numeric_limits<int64_t>::max();
    for (const auto& [subnet, entry] : m_banned) {
        earliest = std::min(earliest, entry.nBanUntil);
    }

    m_next_sweep_time = earliest;
    const uint64_t seq = ++m_sweep_seq;
    const int64_t now = GetTime();
    const auto delay = std::chrono::seconds(std::max<int64_t>(0, earliest - now));
...
void BanMan::Ban(const CSubNet& sub_net, int64_t ban_time_offset, bool since_unix_epoch)
{
    ...
    // Saturate: GetTime() + a large RPC bantime is signed overflow (UB) and
    // wrapping nBanUntil negative makes IsBanned fail open until SweepBanned.
    const int64_t ban_base{normalized_since_unix_epoch ? 0 : GetTime()};
    ban_entry.nBanUntil = SaturatingAdd(ban_base, normalized_ban_time_offset);

    {
        ...
        if (m_banned[sub_net].nBanUntil < ban_entry.nBanUntil) {
            ...
            if (m_sweep_started && ban_entry.nBanUntil < m_next_sweep_time) {
                ...
                const uint64_t seq = ++m_sweep_seq;
                const int64_t now = GetTime();
                const auto delay = std::chrono::seconds(std::max<int64_t>(0, m_next_sweep_time - now));
                m_scheduler->scheduleFromNow([this, seq] { SweepBannedAndSchedule(seq); }, delay);
            }
        } else
            return;
