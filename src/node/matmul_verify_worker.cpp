    ...
    for (;;) {
        ...
        if (!verify_override &&
            ...
            const auto device_priority{[&] {
                ...
                // Acquisition escape is already restricted by chainstate to
                // a bounded, strictly-heavier tower and by net processing to
                // its parent-connectable frontier. Preserve that progress
                // signal on configured mining signers: requiring quorum here
                // would make local ExactReplay depend on the attestation it is
                // supposed to produce. Ordinary siblings never set this flag.
                if (job.acquisition_recovery && protect_body_replay) {
