...
constexpr uint32_t SLABBED_RHS_THREADS{512};
...
bool ShouldStreamProductDigestBIntoFactoredRhs(uint32_t n, uint32_t b)
{
    const char* env = std::getenv("BTX_MATMUL_CUDA_STREAM_B_RHS");
    if (env != nullptr && env[0] == '0') {
        return false;
    }

    const uint32_t blocks_per_axis = n / b;
    return b == 16U && n % 32U == 0U && (blocks_per_axis & 1U) == 0U;
}

bool ShouldUseSplitBLowRankProductDigestRhs(uint32_t n, uint32_t b, uint32_t r, bool stream_b_rhs)
{
    const char* env = std::getenv("BTX_MATMUL_CUDA_SPLIT_B_LOWRANK_RHS");
    if (env != nullptr && env[0] == '0') {
        return false;
    }

    return stream_b_rhs && n == 512U && b == 16U && r == 8U;
}

bool ShouldUseSlabbedSplitBLowRankProductDigestWords(uint32_t n, uint32_t b, uint32_t r, bool stream_b_rhs)
{
    const char* env = std::getenv("BTX_MATMUL_CUDA_SLAB_RHS_WORDS");
...
bool ShouldUseStreamALowRankProductDigestWords(uint32_t n, uint32_t b, uint32_t r, bool stream_b_rhs)
{
    const char* env = std::getenv("BTX_MATMUL_CUDA_STREAM_A_WORDS");
    if (env != nullptr && env[0] == '0') {
        return false;
    }
...
bool ShouldUseStreamARowPairWords()
{
    const char* env = std::getenv("BTX_MATMUL_CUDA_STREAM_A_ROW_PAIR");
...
uint64_t DeviceCapacityBytes(const DigestWorkspace& workspace)
{
    ...
    add(workspace.noise_e_l_capacity, sizeof(Element));
    add(workspace.noise_e_r_capacity, sizeof(Element));
    add(workspace.noise_f_l_capacity, sizeof(Element));
    add(workspace.noise_f_r_capacity, sizeof(Element));
    add(workspace.prepared_input_ptr_capacity, sizeof(Element*));
    add(workspace.compress_capacity, sizeof(Element));
    add(workspace.output_capacity, sizeof(Element));
    add(workspace.digest_capacity, sizeof(DeviceDigestBytes));
    add(workspace.factored_rhs_capacity, sizeof(Element));
...
__device__ inline void SetShaLE32(uint32_t w[16], uint32_t offset, uint32_t value)
{
...
__device__ inline void SetShaBE64(uint32_t w[16], uint32_t offset, uint64_t value)
{
    for (uint32_t i = 0; i < 8U; ++i) {
        const uint32_t shift = (7U - i) * 8U;
        SetShaByte(w, offset + i, static_cast<uint8_t>((value >> shift) & 0xffU));
    }
}
...
__device__ inline void Sha256dElementWords(const Element* words, uint32_t word_count, uint8_t out[32])
{
    uint32_t state[8];
    Sha256Init(state);

    const uint32_t full_blocks = word_count / 16U;
    for (uint32_t block = 0; block < full_blocks; ++block) {
        uint32_t w[16];
        const Element* block_words = words + static_cast<size_t>(block) * 16U;
        #pragma unroll
        for (uint32_t i = 0; i < 16U; ++i) {
            w[i] = Bswap32(block_words[i]);
        }
        Sha256Compress(state, w);
    }

    const uint32_t rem_words = word_count & 15U;
    uint32_t tail[16] = {};
    const Element* tail_words = words + static_cast<size_t>(full_blocks) * 16U;
    for (uint32_t i = 0; i < rem_words; ++i) {
        tail[i] = Bswap32(tail_words[i]);
    }

    const uint32_t rem_bytes = rem_words * 4U;
    SetShaByte(tail, rem_bytes, 0x80U);
    const uint64_t bit_length = static_cast<uint64_t>(word_count) * 32U;
    if (rem_bytes <= 55U) {
        SetShaBE64(tail, 56U, bit_length);
        Sha256Compress(state, tail);
    } else {
        Sha256Compress(state, tail);
        uint32_t length_block[16] = {};
        SetShaBE64(length_block, 56U, bit_length);
        Sha256Compress(state, length_block);
    }

    uint8_t first[32];
    Sha256StateToBytes(state, first);
    Sha256SingleBlock32(first, out);
}

__device__ inline void ProductDigestFromCompressedWords(const Element* words,
                                                       uint32_t word_count,
                                                       const DeviceSeedBytes& sigma,
                                                       uint32_t n,
                                                       uint32_t b,
                                                       DeviceDigestBytes& out)
{
    uint8_t c_prime_hash[32];
    Sha256dElementWords(words, word_count, c_prime_hash);

    constexpr uint8_t PRODUCT_DIGEST_TAG[24] = {
        'm', 'a', 't', 'm', 'u', 'l', '-', 'p',
        'r', 'o', 'd', 'u', 'c', 't', '-', 'd',
        'i', 'g', 'e', 's', 't', '-', 'v', '3'
    };
    ...
    uint32_t block0[16] = {};
    SetShaBytes(block0, 0U, PRODUCT_DIGEST_TAG, 24U);
    SetShaBytes(block0, 24U, sigma.data, 32U);
    SetShaBytes(block0, 56U, c_prime_hash, 8U);
    Sha256Compress(state, block0);

    uint32_t block1[16] = {};
    SetShaBytes(block1, 0U, c_prime_hash + 8U, 24U);
    SetShaLE32(block1, 24U, n);
    SetShaLE32(block1, 28U, b);
    SetShaByte(block1, 32U, 0x80U);
    SetShaBE64(block1, 56U, 96U * 8U);
    Sha256Compress(state, block1);

    uint8_t first[32];
    Sha256StateToBytes(state, first);
    Sha256SingleBlock32(first, out.data);
}
...
// state is identical across all 2^18 elements. PrecomputeSeedPairMidstatesKernel
// computes it once per A/B seed pair (one thread each) into two 16-word records
// (8 packed seed words + 8 state words); the generation kernels load those as
// scalars and resume each element's hash at round 8 - 8 of 64 rounds saved per
// element, with no per-block barrier (which is why a precompute kernel beats a
// shared-memory hoist here). Byte-identical to FromOracle's retry==0 result;
// the ~2^-31 retry/fallback case defers to the full FromOracle path.
...
__device__ __forceinline__ Element PerturbedElementFromSeedMidstatePackedPointer(
    const DeviceSeedBytes& seed,
    const uint32_t* midstate,
    const Element* packed_inputs,
...
{
    const uint32_t candidate = CandidateFromMidstateScalars(midstate, local_index);
    const Element base = candidate < MODULUS ? candidate : FromOracle(seed, local_index);
    ...
    return FieldAdd(base, Reduce64(acc));
}

template <uint32_t MATRIX_N>
__device__ __forceinline__ Element PerturbedElementFromSeedMidstatePackedPointerRank8(
    const DeviceSeedBytes& seed,
    const uint32_t* midstate,
    const Element* packed_inputs,
    uint32_t noise_left_offset_words,
    ...
    uint32_t local_index)
{
    ...
    uint64_t acc =
        static_cast<uint64_t>(noise_left[row * 8U]) * noise_right[col] +
        static_cast<uint64_t>(noise_left[row * 8U + 1U]) * noise_right[MATRIX_N + col] +
        static_cast<uint64_t>(noise_left[row * 8U + 2U]) * noise_right[2U * MATRIX_N + col] +
        static_cast<uint64_t>(noise_left[row * 8U + 3U]) * noise_right[3U * MATRIX_N + col];
    acc = Reduce64(acc);
    acc +=
        static_cast<uint64_t>(noise_left[row * 8U + 4U]) * noise_right[4U * MATRIX_N + col] +
        static_cast<uint64_t>(noise_left[row * 8U + 5U]) * noise_right[5U * MATRIX_N + col] +
        static_cast<uint64_t>(noise_left[row * 8U + 6U]) * noise_right[6U * MATRIX_N + col] +
        static_cast<uint64_t>(noise_left[row * 8U + 7U]) * noise_right[7U * MATRIX_N + col];
    ...
    Element* output_batch)
{
    ...
    output_batch[gid] = PerturbedElementFromSeedMidstatePackedPointer(
        seeds[batch_index],
        midstates + static_cast<size_t>(batch_index) * 16U,
...
__global__ void GeneratePerturbedMatrixPairFromSeedMidstatePackedPointersKernel(
    const DeviceSeedBytes* seeds_a,
    const DeviceSeedBytes* seeds_b,
    ...
    size_t total_matrix_elements,
    uint32_t matrix_elements,
    Element* output_a_batch,
    Element* output_b_batch)
{
    const size_t gid = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (gid >= total_matrix_elements) {
        return;
    ...
    const uint32_t candidate_a = CandidateFromMidstateScalars(
        midstates_a + static_cast<size_t>(batch_index) * 16U,
        local_index);
    const uint32_t candidate_b = CandidateFromMidstateScalars(
        midstates_b + static_cast<size_t>(batch_index) * 16U,
        local_index);
    const Element base_a = candidate_a < MODULUS
        ? candidate_a
        : FromOracle(seeds_a[batch_index], local_index);
    const Element base_b = candidate_b < MODULUS
        ? candidate_b
        : FromOracle(seeds_b[batch_index], local_index);
    ...
    uint32_t pending{0};
    for (uint32_t k = 0; k < r; ++k) {
        acc_a += static_cast<uint64_t>(noise_e_left[row * r + k]) * noise_e_right[k * n + col];
        acc_b += static_cast<uint64_t>(noise_f_left[row * r + k]) * noise_f_right[k * n + col];
        if (++pending == REDUCE_INTERVAL) {
            acc_a = Reduce64(acc_a);
            acc_b = Reduce64(acc_b);
            pending = 0;
        }
    }

    output_a_batch[gid] = FieldAdd(base_a, Reduce64(acc_a));
    output_b_batch[gid] = FieldAdd(base_b, Reduce64(acc_b));
}
...
__global__ void BuildFactoredRhsFromSeedMidstatePackedPointersKernel(
    const DeviceSeedBytes* __restrict__ seeds_b,
    ...
    uint32_t n,
    uint32_t r,
    size_t total_jm_elements,
    uint32_t jm_elements,
    uint32_t rhs_elements,
    Element* __restrict__ rhs_batch)
{
    const size_t gid = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (gid >= total_jm_elements) {
        return;
    }
    const uint32_t batch_index = static_cast<uint32_t>(gid / jm_elements);
    const uint32_t local_index = static_cast<uint32_t>(gid % jm_elements);
    const uint32_t m = local_index % n;
    const uint32_t j = local_index / n;
    const Element* packed_inputs = packed_input_ptrs[batch_index];
    const Element* compress_vec = packed_inputs + compress_offset_words;
    const uint32_t* midstate_b = midstates_b + static_cast<size_t>(batch_index) * 16U;

    uint64_t acc[BLOCK_SIZE] = {};
    uint32_t pending{0};
    #pragma unroll
    for (uint32_t y = 0; y < BLOCK_SIZE; ++y) {
        const uint32_t col = j * BLOCK_SIZE + y;
        const uint32_t matrix_index = m * n + col;
        const Element b_value = PerturbedElementFromSeedMidstatePackedPointer(
            seeds_b[batch_index],
            midstate_b,
            packed_inputs,
            noise_f_left_offset_words,
            noise_f_right_offset_words,
            n,
            r,
            m,
            col,
        ...
        for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
            acc[x] += static_cast<uint64_t>(compress_vec[x * BLOCK_SIZE + y]) * b_value;
        }
        if (++pending == REDUCE_INTERVAL) {
            #pragma unroll
            for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
                acc[x] = Reduce64(acc[x]);
            }
            pending = 0;
        }
    }

    Element* rhs = rhs_batch + static_cast<size_t>(batch_index) * rhs_elements;
    #pragma unroll
    for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
        rhs[(j * BLOCK_SIZE + x) * n + m] = Reduce64(acc[x]);
    }
}

template <uint32_t BLOCK_SIZE, uint32_t MATRIX_N, uint32_t RANK>
__global__ void BuildRhsLowRankRightTransformPackedPointersKernel(
    const Element* const* __restrict__ packed_input_ptrs,
    uint32_t noise_f_right_offset_words,
    uint32_t compress_offset_words,
    uint32_t transform_elements,
    size_t total_transform_elements,
    Element* __restrict__ transform_batch)
{
    const size_t gid = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (gid >= total_transform_elements) {
        return;
    }
    const uint32_t batch_index = static_cast<uint32_t>(gid / transform_elements);
    const uint32_t local_index = static_cast<uint32_t>(gid % transform_elements);
    const uint32_t k = local_index % RANK;
    const uint32_t x = (local_index / RANK) % BLOCK_SIZE;
    const uint32_t j = local_index / (RANK * BLOCK_SIZE);
    const Element* packed_inputs = packed_input_ptrs[batch_index];
    const Element* noise_f_right = packed_inputs + noise_f_right_offset_words;
    const Element* compress_vec = packed_inputs + compress_offset_words;
    ...
    for (uint32_t y = 0; y < BLOCK_SIZE; ++y) {
        const uint32_t col = j * BLOCK_SIZE + y;
        acc += static_cast<uint64_t>(compress_vec[x * BLOCK_SIZE + y]) *
            noise_f_right[k * MATRIX_N + col];
        if (++pending == REDUCE_INTERVAL) {
            acc = Reduce64(acc);
            pending = 0;
        }
    ...
    transform_batch[gid] = Reduce64(acc);
}

template <uint32_t BLOCK_SIZE, uint32_t MATRIX_N, uint32_t RANK>
__global__ void BuildFactoredRhsSplitBLowRankFromSeedMidstatePackedPointersKernel(
    const DeviceSeedBytes* __restrict__ seeds_b,
    const uint32_t* __restrict__ midstates_b,
    const Element* const* __restrict__ packed_input_ptrs,
    const Element* __restrict__ transform_batch,
    uint32_t noise_f_left_offset_words,
    uint32_t compress_offset_words,
    size_t total_jm_elements,
    uint32_t jm_elements,
    ...
    Element* __restrict__ rhs_batch)
{
    ...
    const uint32_t batch_index = static_cast<uint32_t>(gid / jm_elements);
    const uint32_t local_index = static_cast<uint32_t>(gid % jm_elements);
    const uint32_t m = local_index % MATRIX_N;
    const uint32_t j = local_index / MATRIX_N;
    const Element* packed_inputs = packed_input_ptrs[batch_index];
    const Element* noise_f_left = packed_inputs + noise_f_left_offset_words;
    const Element* compress_vec = packed_inputs + compress_offset_words;
    const Element* transform = transform_batch + static_cast<size_t>(batch_index) * transform_elements +
        static_cast<size_t>(j) * BLOCK_SIZE * RANK;
    const uint32_t* midstate_b = midstates_b + static_cast<size_t>(batch_index) * 16U;

    Element f_left[RANK];
    #pragma unroll
    for (uint32_t k = 0; k < RANK; ++k) {
        f_left[k] = noise_f_left[m * RANK + k];
    }

    uint64_t base_acc[BLOCK_SIZE] = {};
    uint32_t pending{0};
    #pragma unroll
    for (uint32_t y = 0; y < BLOCK_SIZE; ++y) {
        const uint32_t col = j * BLOCK_SIZE + y;
        const uint32_t matrix_index = m * MATRIX_N + col;
        const uint32_t candidate = CandidateFromMidstateScalars(midstate_b, matrix_index);
        const Element base_value = candidate < MODULUS
            ? candidate
            : FromOracle(seeds_b[batch_index], matrix_index);
        #pragma unroll
        for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
            base_acc[x] += static_cast<uint64_t>(compress_vec[x * BLOCK_SIZE + y]) * base_value;
        }
        if (++pending == REDUCE_INTERVAL) {
            #pragma unroll
            for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
                base_acc[x] = Reduce64(base_acc[x]);
            }
            pending = 0;
        }
    }
    ...
    #pragma unroll
    for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
        uint64_t low_rank_acc{0};
        uint32_t low_rank_pending{0};
        #pragma unroll
        for (uint32_t k = 0; k < RANK; ++k) {
            low_rank_acc += static_cast<uint64_t>(f_left[k]) * transform[x * RANK + k];
            if (++low_rank_pending == REDUCE_INTERVAL) {
                low_rank_acc = Reduce64(low_rank_acc);
                low_rank_pending = 0;
            }
        }
        rhs[(j * BLOCK_SIZE + x) * MATRIX_N + m] =
            FieldAdd(Reduce64(base_acc[x]), Reduce64(low_rank_acc));
    }
}
...
template <uint32_t BLOCK_SIZE, uint32_t MATRIX_N, uint32_t RANK>
__global__ __launch_bounds__(WORKSPACE_THREADS, 3)
void ComputeFactoredWordsStreamARowBlockKernel(
    const DeviceSeedBytes* __restrict__ seeds_a,
    ...
    Element* __restrict__ output)
{
    static_assert(MATRIX_N % BLOCK_SIZE == 0U);
    constexpr uint32_t BLOCKS_PER_AXIS = MATRIX_N / BLOCK_SIZE;
    constexpr uint32_t WARPS_PER_BLOCK = WORKSPACE_THREADS / 32U;
    __shared__ Element a_block[BLOCK_SIZE * MATRIX_N];

    const uint32_t batch_index = blockIdx.x / BLOCKS_PER_AXIS;
    const uint32_t i = blockIdx.x % BLOCKS_PER_AXIS;
    const uint32_t tid = threadIdx.x;
    const uint32_t lane = tid & 31U;
    const uint32_t warp = tid >> 5U;

    const DeviceSeedBytes& seed_a = seeds_a[batch_index];
    const uint32_t* midstate_a = midstates_a + static_cast<size_t>(batch_index) * 16U;
    const Element* packed_inputs = packed_input_ptrs[batch_index];
    const Element* rhs = rhs_batch + static_cast<size_t>(batch_index) * rhs_elements;

    for (uint32_t idx = tid; idx < BLOCK_SIZE * MATRIX_N; idx += blockDim.x) {
        const uint32_t x = idx / MATRIX_N;
        const uint32_t m = idx % MATRIX_N;
        const uint32_t row = i * BLOCK_SIZE + x;
        if constexpr (RANK == 8U) {
            a_block[idx] = PerturbedElementFromSeedMidstatePackedPointerRank8<MATRIX_N>(
                seed_a,
                midstate_a,
                packed_inputs,
            ...
            a_block[idx] = PerturbedElementFromSeedMidstatePackedPointer(
                seed_a,
                midstate_a,
                packed_inputs,
                noise_e_left_offset_words,
    ...
    constexpr uint32_t J_PAIRS = BLOCKS_PER_AXIS / 2U;
    for (uint32_t j_pair_base = 0; j_pair_base < J_PAIRS; j_pair_base += WARPS_PER_BLOCK) {
        const uint32_t j_pair = j_pair_base + warp;
        if (j_pair >= J_PAIRS) {
            continue;
        }
        const uint32_t j0 = j_pair * 2U;
        const uint32_t j1 = j0 + 1U;
        uint64_t acc0{0};
        uint64_t acc1{0};
        uint32_t pending{0};
        for (uint32_t idx = lane; idx < BLOCK_SIZE * MATRIX_N; idx += 32U) {
            const uint32_t x = idx / MATRIX_N;
            const uint32_t m = idx % MATRIX_N;
            const Element a = a_block[idx];
            const Element d0 = rhs[static_cast<size_t>(j0 * BLOCK_SIZE + x) * MATRIX_N + m];
            const Element d1 = rhs[static_cast<size_t>(j1 * BLOCK_SIZE + x) * MATRIX_N + m];
            acc0 += static_cast<uint64_t>(a) * d0;
            acc1 += static_cast<uint64_t>(a) * d1;
            if (++pending == REDUCE_INTERVAL) {
                acc0 = Reduce64(acc0);
                acc1 = Reduce64(acc1);
                pending = 0;
        ...
        Element value0 = Reduce64(acc0);
        Element value1 = Reduce64(acc1);
        for (uint32_t offset = 16U; offset > 0U; offset >>= 1U) {
            value0 = FieldAdd(value0, __shfl_down_sync(0xffffffffU, value0, offset));
            value1 = FieldAdd(value1, __shfl_down_sync(0xffffffffU, value1, offset));
        }
        if (lane == 0) {
            const uint32_t base = batch_index * words_per_request + i * BLOCKS_PER_AXIS;
            output[base + j0] = value0;
            output[base + j1] = value1;
        }
    }
}
...
void ComputeFactoredWordsStreamARowPairKernel(
    const DeviceSeedBytes* __restrict__ seeds_a,
    const uint32_t* __restrict__ midstates_a,
    const Element* const* __restrict__ packed_input_ptrs,
    const Element* __restrict__ rhs_batch,
    ...
    Element* __restrict__ output)
{
    static_assert(MATRIX_N % BLOCK_SIZE == 0U);
    constexpr uint32_t BLOCKS_PER_AXIS = MATRIX_N / BLOCK_SIZE;
    constexpr uint32_t ROW_PAIRS_PER_REQUEST = BLOCKS_PER_AXIS / 2U;
    constexpr uint32_t WARPS_PER_BLOCK = WORKSPACE_THREADS / 32U;
    extern __shared__ Element a_pair[];

    const uint32_t batch_index = blockIdx.x / ROW_PAIRS_PER_REQUEST;
    const uint32_t i_pair = blockIdx.x % ROW_PAIRS_PER_REQUEST;
    const uint32_t i0 = i_pair * 2U;
    const uint32_t tid = threadIdx.x;
    const uint32_t lane = tid & 31U;
    const uint32_t warp = tid >> 5U;
    ...
    for (uint32_t idx = tid; idx < 2U * BLOCK_SIZE * MATRIX_N; idx += blockDim.x) {
        const uint32_t x = idx / MATRIX_N;
        const uint32_t m = idx % MATRIX_N;
        const uint32_t row = i0 * BLOCK_SIZE + x;
        a_pair[idx] = PerturbedElementFromSeedMidstatePackedPointer(
            seed_a,
            midstate_a,
            packed_inputs,
            noise_e_left_offset_words,
    ...
    for (uint32_t j_pair_base = 0; j_pair_base < J_PAIRS; j_pair_base += WARPS_PER_BLOCK) {
        ...
        uint32_t pending{0};
        for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
            const Element* row0 = a_pair + static_cast<size_t>(x) * MATRIX_N;
            const Element* row1 = row0 + static_cast<size_t>(BLOCK_SIZE) * MATRIX_N;
            const Element* d0 = rhs + static_cast<size_t>(j0 * BLOCK_SIZE + x) * MATRIX_N;
            const Element* d1 = rhs + static_cast<size_t>(j1 * BLOCK_SIZE + x) * MATRIX_N;
            for (uint32_t m = lane; m < MATRIX_N; m += 32U) {
                const Element a0 = row0[m];
                const Element a1 = row1[m];
                const Element b0 = d0[m];
                const Element b1 = d1[m];
                acc00 += static_cast<uint64_t>(a0) * b0;
                acc01 += static_cast<uint64_t>(a0) * b1;
                acc10 += static_cast<uint64_t>(a1) * b0;
                acc11 += static_cast<uint64_t>(a1) * b1;
                if (++pending == REDUCE_INTERVAL) {
                    acc00 = Reduce64(acc00);
                    acc01 = Reduce64(acc01);
                    acc10 = Reduce64(acc10);
        ...
        if (lane == 0) {
            const uint32_t base = batch_index * words_per_request;
            output[base + i0 * BLOCKS_PER_AXIS + j0] = value00;
            output[base + i0 * BLOCKS_PER_AXIS + j1] = value01;
            output[base + (i0 + 1U) * BLOCKS_PER_AXIS + j0] = value10;
            output[base + (i0 + 1U) * BLOCKS_PER_AXIS + j1] = value11;
        }
    }
}
...
void ComputeFactoredWordsSplitBLowRankSlabbedKernel(
    const Element* __restrict__ matrix_a_batch,
    const DeviceSeedBytes* __restrict__ seeds_b,
    const uint32_t* __restrict__ midstates_b,
    const Element* const* __restrict__ packed_input_ptrs,
...
{
    static_assert(MATRIX_N % BLOCK_SIZE == 0U);
    static_assert((MATRIX_N / BLOCK_SIZE) % 2U == 0U);
    static_assert(SLAB_M % 32U == 0U);
    static_assert(MATRIX_N % SLAB_M == 0U);

    constexpr uint32_t BLOCKS_PER_AXIS = MATRIX_N / BLOCK_SIZE;
    constexpr uint32_t TILES_PER_AXIS = BLOCKS_PER_AXIS / 2U;
    __shared__ Element rhs_slab[2U * BLOCK_SIZE * SLAB_M];

    const uint32_t batch_index = blockIdx.x / TILES_PER_AXIS;
    const uint32_t j_pair = blockIdx.x % TILES_PER_AXIS;
    const uint32_t j0 = j_pair * 2U;
    const uint32_t tid = threadIdx.x;
    const uint32_t lane = tid & 31U;
    const uint32_t warp = tid >> 5U;
    const uint32_t i0 = warp * 2U;
    ...
    const Element* compress_vec = packed_inputs + compress_offset_words;
    const uint32_t* midstate_b = midstates_b + static_cast<size_t>(batch_index) * 16U;
    const Element* transform_pair = transform_batch + static_cast<size_t>(batch_index) * transform_elements +
        static_cast<size_t>(j0) * BLOCK_SIZE * RANK;
    ...
    for (uint32_t slab_base = 0; slab_base < MATRIX_N; slab_base += SLAB_M) {
        if (tid < 2U * SLAB_M) {
            const uint32_t rel_j = tid / SLAB_M;
            const uint32_t local_m = tid % SLAB_M;
            const uint32_t m = slab_base + local_m;
            const uint32_t j = j0 + rel_j;
            const Element* transform = transform_pair + static_cast<size_t>(rel_j) * BLOCK_SIZE * RANK;
            ...
            for (uint32_t y = 0; y < BLOCK_SIZE; ++y) {
                ...
                if (++base_pending == REDUCE_INTERVAL) {
                    #pragma unroll
                    for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
                        base_acc[x] = Reduce64(base_acc[x]);
                    }
            ...
            for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
                ...
                rhs_slab[(rel_j * BLOCK_SIZE + x) * SLAB_M + local_m] =
                    FieldAdd(Reduce64(base_acc[x]), Reduce64(low_rank_acc));
            }
        }
        __syncthreads();
        ...
        #pragma unroll
        for (uint32_t x = 0; x < BLOCK_SIZE; ++x) {
            const Element* a_row0 = matrix_a + static_cast<size_t>(i0 * BLOCK_SIZE + x) * MATRIX_N;
            const Element* a_row1 = a_row0 + static_cast<size_t>(BLOCK_SIZE) * MATRIX_N;
            const Element* d_row0 = rhs_slab + static_cast<size_t>(x) * SLAB_M;
            const Element* d_row1 = d_row0 + static_cast<size_t>(BLOCK_SIZE) * SLAB_M;
            for (uint32_t local_m = lane; local_m < SLAB_M; local_m += 32U) {
                const uint32_t m = slab_base + local_m;
                const uint64_t a0 = a_row0[m];
                const uint64_t a1 = a_row1[m];
                const uint64_t d0 = d_row0[local_m];
                const uint64_t d1 = d_row1[local_m];
                acc00 += a0 * d0;
                acc01 += a0 * d1;
                acc10 += a1 * d0;
                acc11 += a1 * d1;
...
__global__ void ComputeProductDigestsKernel(const Element* __restrict__ words,
                                            ...
                                            DeviceDigestBytes* __restrict__ digests)
{
    ...
    ProductDigestFromCompressedWords(
        words + static_cast<size_t>(batch_index) * words_per_request,
        words_per_request,
        sigmas[batch_index],
        n,
        b,
...
bool ValidateLowRankVariableBaseProductDigestDeviceBatchRequest(
    const MatMulLowRankVariableBaseProductDigestDeviceBatchRequest& request,
    std::string& error)
{
    if (request.batch_size == 0) {
        error = "CUDA variable-base product digest batch request requires at least one entry";
        return false;
    }
    if (request.n == 0 || request.b == 0 || request.r == 0) {
        error = "matrix dimension, transcript block size, and noise rank must be non-zero";
    ...
    if (request.matrix_a_seeds == nullptr ||
        request.matrix_b_seeds == nullptr ||
        request.sigmas == nullptr ||
        request.generated_inputs == nullptr) {
        error = "CUDA variable-base product digest batch request requires matrix seeds, sigmas, and device-generated inputs";
        return false;
    }
    for (uint32_t i = 0; i < request.batch_size; ++i) {
        if (request.generated_inputs[i] == nullptr) {
            error = "CUDA variable-base product digest batch request contains null device-generated input handles";
            return false;
        }
    }
    return true;
...
bool FinalizeProductDigestsPackedPointerBatch(const BufferPoolLease& lease,
                                              DigestWorkspace& workspace,
                                              uint32_t n,
                                              uint32_t b,
                                              uint32_t r,
                                              ...
                                              bool& allocated_buffers)
{
    ...
    const uint32_t matrix_elements = n * n;
    const uint32_t pair_count_per_request = blocks_per_axis * blocks_per_axis;
    const uint32_t words_per_request = pair_count_per_request;
    const size_t total_output_count = static_cast<size_t>(batch_size) * words_per_request;
    ...
    const uint32_t total_pair_count = batch_size * pair_count_per_request;
    const auto finalize_start = SteadyClock::now();
    if (n % 32U == 0U && (blocks_per_axis & 1U) == 0U) {
        const uint32_t rhs_elements = b * n * blocks_per_axis;
        const size_t total_rhs_elements = static_cast<size_t>(batch_size) * rhs_elements;
        const bool split_b_low_rank_rhs = ShouldUseSplitBLowRankProductDigestRhs(n, b, r, stream_b_rhs);
        const bool slabbed_split_b_words =
            !stream_a_words && split_b_low_rank_rhs && ShouldUseSlabbedSplitBLowRankProductDigestWords(n, b, r, stream_b_rhs);
        if (slabbed_split_b_words) {
            const uint32_t transform_elements = blocks_per_axis * b * r;
            const size_t total_transform_elements = static_cast<size_t>(batch_size) * transform_elements;
            if (!EnsureDeviceBuffer(workspace.device_matrix_b,
                                    workspace.matrix_b_capacity,
                                    total_transform_elements,
                                    result.error,
                                    allocated_buffers)) {
                return false;
            }
            const uint32_t transform_blocks = static_cast<uint32_t>(
                (total_transform_elements + WORKSPACE_THREADS - 1) / WORKSPACE_THREADS);
            BuildRhsLowRankRightTransformPackedPointersKernel<16, 512, 8><<<transform_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
                workspace.device_prepared_input_ptrs,
                noise_f_right_offset_words,
                compress_offset_words,
                transform_elements,
                total_transform_elements,
                workspace.device_matrix_b);
            const uint32_t j_pair_count_per_request = blocks_per_axis >> 1U;
            const uint32_t word_blocks = batch_size * j_pair_count_per_request;
            ComputeFactoredWordsSplitBLowRankSlabbedKernel<16, 512, 8, 128><<<word_blocks, SLABBED_RHS_THREADS, 0, workspace.stream>>>(
                workspace.device_matrix_a,
                workspace.device_seed_b,
                workspace.device_seed_midstates + static_cast<size_t>(batch_size) * 16U,
                workspace.device_prepared_input_ptrs,
                workspace.device_matrix_b,
                noise_f_left_offset_words,
                compress_offset_words,
            ...
            const uint32_t rhs_blocks = static_cast<uint32_t>(
                (total_rhs_elements + WORKSPACE_THREADS - 1) / WORKSPACE_THREADS);
            if (split_b_low_rank_rhs) {
                const uint32_t jm_elements = blocks_per_axis * n;
                const size_t total_jm_elements = static_cast<size_t>(batch_size) * jm_elements;
                const uint32_t transform_elements = blocks_per_axis * b * r;
                const size_t total_transform_elements = static_cast<size_t>(batch_size) * transform_elements;
                if (!EnsureDeviceBuffer(workspace.device_matrix_b,
                                        workspace.matrix_b_capacity,
                ...
                BuildRhsLowRankRightTransformPackedPointersKernel<16, 512, 8><<<transform_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
                    ...
                    total_transform_elements,
                    workspace.device_matrix_b);
                const uint32_t jm_blocks = static_cast<uint32_t>(
                    (total_jm_elements + WORKSPACE_THREADS - 1) / WORKSPACE_THREADS);
                BuildFactoredRhsSplitBLowRankFromSeedMidstatePackedPointersKernel<16, 512, 8><<<jm_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
                    workspace.device_seed_b,
                    workspace.device_seed_midstates + static_cast<size_t>(batch_size) * 16U,
                    workspace.device_prepared_input_ptrs,
                    workspace.device_matrix_b,
                    ...
                    transform_elements,
                    workspace.device_factored_rhs);
            } else if (stream_b_rhs && b == 16U) {
                const uint32_t jm_elements = blocks_per_axis * n;
                const size_t total_jm_elements = static_cast<size_t>(batch_size) * jm_elements;
                const uint32_t jm_blocks = static_cast<uint32_t>(
                    (total_jm_elements + WORKSPACE_THREADS - 1) / WORKSPACE_THREADS);
                BuildFactoredRhsFromSeedMidstatePackedPointersKernel<16><<<jm_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
                    workspace.device_seed_b,
                    workspace.device_seed_midstates + static_cast<size_t>(batch_size) * 16U,
                    workspace.device_prepared_input_ptrs,
                    noise_f_left_offset_words,
            ...
            const uint32_t word_blocks = (total_tile_count + warps_per_block - 1U) / warps_per_block;
            if (stream_a_words) {
                constexpr uint32_t row_pair_shared_elements = 2U * 16U * 512U;
                constexpr uint32_t row_pair_shared_bytes = row_pair_shared_elements * sizeof(Element);
                const bool row_pair_enabled = ShouldUseStreamARowPairWords();
                cudaError_t pending_launch_error = cudaSuccess;
                cudaError_t attr_error = cudaErrorInvalidValue;
                if (row_pair_enabled) {
                    pending_launch_error = cudaPeekAtLastError();
                    attr_error = pending_launch_error == cudaSuccess
                        ? cudaFuncSetAttribute(
                        ComputeFactoredWordsStreamARowPairKernel<16, 512, 8>,
                        cudaFuncAttributeMaxDynamicSharedMemorySize,
                        static_cast<int>(row_pair_shared_bytes))
                        : pending_launch_error;
                }
                if (attr_error == cudaSuccess) {
                    const uint32_t row_pair_blocks = batch_size * (blocks_per_axis >> 1U);
                    ComputeFactoredWordsStreamARowPairKernel<16, 512, 8><<<row_pair_blocks, WORKSPACE_THREADS, row_pair_shared_bytes, workspace.stream>>>(
                    ...
                    if (row_pair_enabled && pending_launch_error == cudaSuccess) {
                        (void)cudaGetLastError();
                    }
                    const uint32_t row_blocks = batch_size * blocks_per_axis;
                    ComputeFactoredWordsStreamARowBlockKernel<16, 512, 8><<<row_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
                        workspace.device_seed_a,
                        workspace.device_seed_midstates,
                        workspace.device_prepared_input_ptrs,
                        workspace.device_factored_rhs,
    ...
    ComputeProductDigestsKernel<<<digest_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
        workspace.device_output,
    ...
    sample.submit_d2d_us += DurationMicros(digest_start, SteadyClock::now());
    if (error != cudaSuccess) {
        result.error = "CUDA product digest hash kernel failed:" + CudaErrorString(error);
        return false;
    }

    std::vector<DeviceDigestBytes> host_digests(batch_size);
    const auto d2h_start = SteadyClock::now();
    error = cudaMemcpyAsync(host_digests.data(),
                            workspace.device_digests,
                            batch_size * sizeof(DeviceDigestBytes),
                            cudaMemcpyDeviceToHost,
                            workspace.stream);
    sample.submit_d2h_us = DurationMicros(d2h_start, SteadyClock::now());
    const auto sync_start = SteadyClock::now();
    ...
    sample.stream_sync_us = DurationMicros(sync_start, SteadyClock::now());
    if (error != cudaSuccess) {
        result.error = "CUDA product digest stream completion failed:" + CudaErrorString(error);
        return false;
    }
    ...
    int device_index)
{
    ...
    const uint32_t build_blocks = static_cast<uint32_t>((total_matrix_elements + WORKSPACE_THREADS - 1) / WORKSPACE_THREADS);
    // Per-seed SHA midstate buffer (16 words/seed), with A records followed by
    // B records so one launch can precompute both seed streams.
    const uint32_t seed_midstate_words = request.batch_size * 32U;
    ...
    error = cudaGetLastError();
    if (error == cudaSuccess) {
        GeneratePerturbedMatrixPairFromSeedMidstatePackedPointersKernel<<<build_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
            workspace.device_seed_a,
            workspace.device_seed_b,
            workspace.device_seed_midstates,
    ...
    int device_index)
{
    ...
    if (!ValidateLowRankVariableBaseProductDigestDeviceBatchRequest(request, result.error)) {
        return result;
    }
    ...
    const uint32_t noise_f_r_offset_words = noise_f_l_offset_words + noise_left_elements;
    const uint32_t compress_offset_words = noise_f_r_offset_words + noise_right_elements;
    const bool stream_b_rhs = ShouldStreamProductDigestBIntoFactoredRhs(request.n, request.b);
    const bool stream_a_words = ShouldUseStreamALowRankProductDigestWords(
        request.n,
        request.b,
        request.r,
        stream_b_rhs);
    if (stream_a_words) {
        sample.mode = "product_digest_stream_a";
    }
    bool allocated_buffers{false};
    ...
    for (uint32_t i = 0; i < request.batch_size; ++i) {
        ...
        if (generated->device_index != runtime.device_index ||
            ...
            generated->noise_f_r == nullptr ||
            generated->compress_vec == nullptr) {
            result.error = "CUDA variable-base product digest batch request contains incompatible device-generated inputs";
            return result;
        }
    ...
    sample.submit_h2d_us = DurationMicros(h2d_start, SteadyClock::now());
    if (error != cudaSuccess) {
        result.error = "cudaMemcpy variable-base product digest request data failed:" + CudaErrorString(error);
        return result;
    }
    ...
    PrecomputeSeedPairMidstatesKernel<<<midstate_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
        ...
        workspace.device_seed_midstates);
    error = cudaGetLastError();
    if (error == cudaSuccess && stream_a_words) {
        // A' is reconstructed inside the word kernel. The B-side RHS builder
        // still uses the B seed midstate below, so no full matrix batch is
        // needed here.
    } else if (error == cudaSuccess && stream_b_rhs) {
        GeneratePerturbedMatrixFromSeedMidstatePackedPointersKernel<<<build_blocks, WORKSPACE_THREADS, 0, workspace.stream>>>(
            workspace.device_seed_a,
            workspace.device_seed_midstates,
            workspace.device_prepared_input_ptrs,
            noise_e_l_offset_words,
    ...
    sample.launch_build_perturbed_us = DurationMicros(build_start, SteadyClock::now());
    if (error != cudaSuccess) {
        result.error = "CUDA variable-base product digest matrix kernel failed:" + CudaErrorString(error);
        return result;
    }

    if (!FinalizeProductDigestsPackedPointerBatch(
            *lease,
            workspace,
            request.n,
            request.b,
...
MatMulProductDigestBatchResult ComputeProductDigestsLowRankVariableBaseDeviceBatch(
    const MatMulLowRankVariableBaseProductDigestDeviceBatchRequest& request)
{
    return ComputeProductDigestsLowRankVariableBaseDeviceBatchMultiDevice(request);
}
    ...
    const MatMulLowRankVariableBaseProductDigestDeviceBatchRequest& request)
{
    ...
    if (shard_map.size() == 1) {
        return ComputeProductDigestsLowRankVariableBaseDeviceBatchOnDevice(
            request,
            shard_map.begin()->first);
    }
    ...
    for (const auto& entry : shard_map) {
        ...
        futures.push_back(std::async(std::launch::async, [request, shard]() {
            const MatMulLowRankVariableBaseProductDigestDeviceBatchRequest shard_request{
                .n = request.n,
                .b = request.b,
                .r = request.r,
                .batch_size = static_cast<uint32_t>(shard.inputs.size()),
                .matrix_a_seeds = shard.seed_a.data(),
                .matrix_b_seeds = shard.seed_b.data(),
                .sigmas = shard.sigmas.data(),
                .generated_inputs = shard.inputs.data(),
            };
            return ShardResult{
                .shard = shard,
                .result = ComputeProductDigestsLowRankVariableBaseDeviceBatchOnDevice(
                    shard_request,
                    shard.device_index),
            };
        }));
    ...
    try {
        ...
        result.error = std::string{"cuda_multi_device_variable_base_product_digest_batch_exception:"} + e.what();
        return result;
    }
    ...
    for (const auto& shard_result : shard_results) {
        if (!shard_result.result.success) {
            result.available = shard_result.result.available;
            result.error = "cuda_device_" + std::to_string(shard_result.shard.device_index) + "_variable_base_product_digest_batch_failed:" +
                (shard_result.result.error.empty() ? "unknown_error" : shard_result.result.error);
            return result;
        }
    }

    result.digests.assign(request.batch_size, uint256{});
    for (const auto& shard_result : shard_results) {
        if (shard_result.result.digests.size() != shard_result.shard.inputs.size() ||
            shard_result.shard.indices.size() != shard_result.shard.inputs.size()) {
            result.success = false;
            result.error = "cuda_multi_device_variable_base_product_digest_batch_result_size_mismatch";
            result.digests.clear();
            return result;
        }
        for (size_t i = 0; i < shard_result.shard.indices.size(); ++i) {
            result.digests[shard_result.shard.indices[i]] = shard_result.result.digests[i];
        }
    }
