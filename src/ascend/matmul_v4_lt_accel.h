...
//
// Grounded in CANN ≥ 9.1 / Ascend 950PR·DT (dav-3510) aclnn samples:
//   aclnnMatmul / aclnnMm / aclnnMatmulWeightNz + INT8 via aclnnTransMatmulWeight.
// Two-phase GetWorkspaceSize + execute; cubeMathType KEEP_DTYPE (no HF32 /
// down-precision). `used_cube_path` is set ONLY after a Cube/aclnn path ran
// AND matched ExactGemmS8S8 byte-for-byte.
//
// Without CANN (default CI / BTX_ENABLE_ASCEND=OFF): stub declines — never
// pretends Cube ran. Public activation remains inert (INT32_MAX).
// ---------------------------------------------------------------------------
 ...
 *  non-null it is set true ONLY when the Cube/aclnn datapath executed and
 *  matched; otherwise false. Returns false → caller MUST use CPU ExactGemm. */
...
/** ExactGemmS32S8Ascend — scaffolding declines until a proven exact Cube path
 *  exists; callers fall back to CPU ExactGemmS32S8. */
