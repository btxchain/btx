...
mlkem::PublicKey ParseMLKEMPublicKey(std::string_view value, std::string_view option_name)
{
    const auto bytes = ParseHex(std::string{value});
    if (bytes.size() != mlkem::PUBLICKEYBYTES) {
        throw std::runtime_error(std::string{option_name} + " must be a full ML-KEM public key hex string");
    }
    mlkem::PublicKey out{};
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help") {
            std::cout << "Usage: gen_shielded_spend_path_recovery_fixture "
                         ...
                         "[--recovery-recipient-pk-hash=<hex>] "
                         "[--recovery-recipient-kem-public-key=<hex>] "
        ...
        if (arg.starts_with("--recovery-recipient-pk-hash=")) {
            const auto hash = uint256::FromHex(std::string{arg.substr(29)});
            if (!hash.has_value()) {
                throw std::runtime_error("invalid --recovery-recipient-pk-hash");
            }
            parsed.recovery_recipient_pk_hash = *hash;
            continue;
        }
        if (arg.starts_with("--recovery-recipient-kem-public-key=")) {
            parsed.recovery_recipient_kem_public_key =
                ParseMLKEMPublicKey(arg.substr(36), "--recovery-recipient-kem-public-key");
            continue;
        }
