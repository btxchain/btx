...
// (c) ADDITIVE GUARD: legacy profile 1 resolver output is byte-identical to
//     today's DefaultConsensusRCEpisodeParams(), and datacenter changes only
//     the explicitly frozen profile axes.
BOOST_AUTO_TEST_CASE(rc_dc_episode_profile1_byte_identical_guard)
{
    ...
    // MatMul v4.7 Profile 1 stays byte-identical to the pre-datacenter base.
    Consensus::Params p1;
...
BOOST_AUTO_TEST_CASE(rc_dc_episode_profile_selector_default_and_mainnet)
{
    ...
    BOOST_CHECK(main.IsMatMulRCActive(182'600));
    BOOST_CHECK(RCEpisodeParamsEqual(rc::ResolveRCEpisodeParams(main, 0),
                                     rc::DefaultConsensusRCEpisodeParams()));

    // Profile 2 remains explicitly selectable and resolves to datacenter dims.
    Consensus::Params dc_selected;
    dc_selected.nMatMulRCProfile = 2;
    BOOST_CHECK(RCEpisodeParamsEqual(rc::ResolveRCEpisodeParams(dc_selected, 0),
...
// (e) REGTEST ACTIVATION: -regtestrcprofile=2 plus a finite, unified RC-family
//     height makes the resolved episode the datacenter dims, and the RC family
//     activates at that height (mirrors rc_coup_unified_height_switch).
BOOST_AUTO_TEST_CASE(rc_dc_episode_regtest_profile2_activation)
{
    ArgsManager args;
    args.ForceSetArg("-regtestrcprofile", "2");
    args.ForceSetArg("-regtestrcunifiedheight", "150");
    ...
    // MatMul v4.7 regtest defaults Profile 1 RC to height 101 with toy
    // dimensions, while Profile 2 remains an explicit regression override.
    const auto reg_def =
        CreateChainParams(ArgsManager{}, ChainType::REGTEST)->GetConsensus();
    BOOST_CHECK_EQUAL(reg_def.nMatMulRCProfile, 1u);
    BOOST_CHECK_EQUAL(reg_def.nMatMulRCHeight, 101);
    BOOST_CHECK(!reg_def.IsMatMulRCActive(0));
    BOOST_CHECK(reg_def.IsMatMulRCActive(101));
    BOOST_CHECK(reg_def.fMatMulRCUseToyDims);
...
// (c-FVT) Preserve the sampled-carrier last-round grinding regression, but
//   require epsilon-zero ExactReplay to reject it regardless of the legacy FVT
//   policy flag.
//
...
//   proves the gap is real); (2) CheckMatMulProofOfWork_RC rejects it with FVT
//   either disabled or enabled because every round is replayed; and (3) an
//   honest carrier still passes.
BOOST_AUTO_TEST_CASE(rc_dc_fvt_rejects_lastround_grind_accepts_honest)
{
    ...
    // and execution actually reaches the optional precheck. Overriding
    // rounds/L_lyr on a local copy (as an earlier draft did) mismatches the
    // consensus-resolved toy dims and the carrier is ignored at the shape gate.
    ...
    // --- Baseline: the HONEST carrier and exact replay both pass. ---
    {
    ...
    // is exactly the mismatch the legacy FVT policy was meant to catch.
    ...
    // --- Legacy FVT flag OFF: complete ExactReplay rejects the cheat. ---
    {
    ...
    // --- Legacy FVT flag ON: ExactReplay remains the rejecting authority. ---
    {
        rc::RCFreivaldsCarrierStoreClear();
        rc::RCFreivaldsCarrierStorePut(cheat_header.GetHash(), cheat_carrier);
...
// (e) The construction invariant still ACCEPTS an explicitly selected
//     datacenter config (profile 2 + finite regtest height + coupled 16× ASERT) — building
