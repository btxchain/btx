...
/** Contiguous, alignment-sized canonical partition of [0, rows). */
[[nodiscard]] std::vector<RCRowRange> CanonicalRCRowRanges(
    uint32_t rows, uint32_t shard_count, uint32_t alignment = 32);

/**
 * Transport-neutral trusted-host request. Shared immutable operands let an
 * in-process executor avoid multi-GiB copies; an RPC adapter may serialize the
 * same fields or replace weight bytes with a preinstalled cache identifier.
 */
struct RCFfnRowShardRequest {
    uint256 episode_id{};
    ...
    uint256 prf_up{};
    uint256 prf_down{};
    std::shared_ptr<const std::vector<int8_t>> x_rows;
    std::shared_ptr<const std::vector<int8_t>> w_up;
    std::shared_ptr<const std::vector<int8_t>> w_down;
};

struct RCFfnRowShardResponse {
    uint256 request_commitment{};
...
using RCFfnRowShardExecutor =
    std::function<RCFfnRowShardResponse(const RCFfnRowShardRequest&)>;

[[nodiscard]] uint256 CommitRCFfnRowShardRequest(
    const RCFfnRowShardRequest& request);
[[nodiscard]] uint256 CommitRCFfnRowShardOutput(
    const RCFfnRowShardRequest& request,
    const std::vector<int8_t>& output_rows);

/** CPU/device-local executor used by trusted workers and differential tests. */
[[nodiscard]] RCFfnRowShardResponse ExecuteRCFfnRowShardLocal(
    const RCFfnRowShardRequest& request,
    const matmul::v4::lt::ExactGemmBackend& backend = {});

struct RCDistributedFfnResult {
    std::vector<int8_t> output;
    std::vector<uint256> request_commitments;
...
/**
 * Dispatch one complete FFN layer concurrently across trusted executors,
 * verify exact ranges/commitments, and concatenate in canonical row order.
 */
[[nodiscard]] RCDistributedFfnResult ExecuteTrustedRCFfnLayer(
