...
struct ExactGemmBackend {
    ...
    using S32S8Fn = bool (*)(const std::vector<int32_t>& L, const std::vector<int8_t>& R,
                             uint32_t rows, uint32_t inner, uint32_t cols,
                             std::vector<int32_t>& out);
    /** Optional RC-only fused FFN lane. The provider owns both exact GEMMs and
     *  the two consensus ExtractMX operations, returning only the committed
     *  int8 output to the host. Null keeps the portable ExactGemm composition. */
    using RCFusedFfnFn = bool (*)(
    ...
    /** Optional RC-only resident multi-layer lane. The provider uploads every
     * layer's immutable weights once, keeps activations on device across the
     * chain, batches command buffers, and returns canonical per-layer outputs.
     * A single weight pair denotes episode-shared weights. */
    using RCFusedFfnChainFn = bool (*)(
    ...
    /** Optional RC tile-tree lanes. Leaves are canonical T_leaf-byte payloads;
     * the provider supplies the 0x00 domain byte and SHA256d. Root folding
     * receives an already power-of-two padded leaf vector and supplies the
     * 0x01 domain byte at every level. */
    using RCMerkleLeavesFn = bool (*)(
