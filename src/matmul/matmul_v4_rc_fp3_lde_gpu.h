...
//
// Provider-neutral Stage-3 Fp3 NTT/LDE accelerator (CUDA and/or Apple Metal).
//
...
// request, and -2 for a device allocation/compilation/execution failure.
//
// BtxGpuFp3Lde* is the selection ABI used by the prover (BTX_GPU_LDE). CUDA
// and Metal each provide it exclusively per build (same pattern as
// BtxGpuRowLeaf*). BtxMetalFp3Lde* remains for Metal-specific tests and is a
// thin alias of the Metal provider.
...
/** 1 iff a usable CUDA or Apple Metal Fp3 LDE provider is present. */
int BtxGpuFp3LdeAvailable(void);
 ...
 * representatives; the device canonicalizes them before use.
 */
int BtxGpuFp3LdeBegin(uint32_t domain_size,
 ...
 */
int BtxGpuFp3LdeForward(void* ctx,
