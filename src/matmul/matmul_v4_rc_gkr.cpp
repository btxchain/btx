    ...
    RCExactReplayAcceleration acceleration)
{
    ...
    out.proof_bytes = 0;
    out.acceleration_backend = acceleration_stats.backend;
    out.fully_accelerated = acceleration_stats.fully_accelerated;
    out.full_metal_pipeline =
    ...
    if (header.matmul_digest.IsNull() || out.digest != header.matmul_digest) {
        // A device result is never the sole basis for permanently rejecting
        // and punishing a peer. Re-run the exact portable oracle after a
        // device mismatch; this is expensive, but admission policy has
