...
bool AppendChildConstraintSystemV1(
    aq::AirConstraintSystem<gf::Fp3>& parent_cs,
    const aq::AirConstraintSystem<gf::Fp3>& child_cs,
    uint32_t child_ordinal,
    ChildAttachmentV1& out,
    std::string* why)
{
    ...
    if (child_cs.n_rows < 2 ||
        (child_cs.n_rows &
         (child_cs.n_rows - 1U)) != 0 ||
        child_cs.n_columns == 0 ||
        !child_cs
             .preprocessed_row_group_roots.empty() ||
        !parent_cs
             .preprocessed_row_group_roots.empty()) {
        return Fail(why, "public_child_shape");
    }
    if (parent_cs.n_columns == 0) {
        if (parent_cs.n_rows != 0 ||
            ...
            !parent_cs.preprocessed_roots.empty()) {
            return Fail(
                why, "public_noncanonical_empty_parent");
        }
        parent_cs.n_rows = child_cs.n_rows;
    } else if (parent_cs.n_rows != child_cs.n_rows) {
        return Fail(
            why, "public_row_count_mismatch");
    }
    if (child_cs.n_columns >
        std::numeric_limits<uint32_t>::max() -
            parent_cs.n_columns) {
        return Fail(
            why, "public_column_count_overflow");
    }
    ...
    if (!constraint_bytecode::
            AppendRelocatedAirConstraintsV1(
                ...
                parent_cs, relocation, why)) {
        return Fail(
            why, "public_child_constraint_relocation");
    }
    out.canonical_constraints_relocated =
        relocation.canonical_constraints_relocated;
    out.native_constraints_shifted =
        relocation.native_constraints_shifted;
    out.canonical_tables_recommitted =
        relocation.canonical_tables_recommitted;
    out.canonical_program_relocation_exact =
...
bool AppendChildLiftedConstraintSystemV1(
    aq::AirConstraintSystem<gf::Fp3>& parent_cs,
    const aq::AirConstraintSystem<gf::Fp3>& child_cs,
    uint32_t parent_rows,
    uint32_t child_ordinal,
    ...
    std::string* why)
{
    if (child_cs.n_rows < 2 ||
        (child_cs.n_rows &
         (child_cs.n_rows - 1U)) != 0 ||
        child_cs.n_columns == 0) {
        return Fail(why, "public_lift_child_shape");
    }
    if (parent_rows < child_cs.n_rows ||
        (parent_rows & (parent_rows - 1U)) != 0 ||
        (parent_rows % child_cs.n_rows) != 0) {
        return Fail(why, "public_lift_row_domain");
    }
    if (!child_cs.preprocessed_roots.empty() ||
        !child_cs
             .preprocessed_row_group_roots.empty()) {
        return Fail(
            why,
            "public_lift_commitment_requires_global_rebuild");
    }
    if (child_cs.n_columns >
        (std::numeric_limits<uint32_t>::max() -
         5U) /
            2U) {
        return Fail(
            why, "public_lift_column_count_overflow");
    }
    for (const auto& constraint :
         child_cs.constraints) {
        if (!constraint.eval ||
            ...
            constraint.alg_degree ==
                std::numeric_limits<uint32_t>::max()) {
            return Fail(
                why, "public_lift_child_constraint");
        }
    }
    ...
    for (const auto& [column, values] :
        ...
        if (column >= child_cs.n_columns ||
            values.size() != child_cs.n_rows) {
            return Fail(
                why, "public_lift_preprocessed_shape");
        }
        std::vector<gf::Fp3> padded(
            parent_rows, gf::Fp3::Zero());
        std::copy(
    ...
    std::array<std::vector<gf::Fp3>, kSelectors>
        selectors;
    ...
    for (uint32_t row = 0;
         row < parent_rows; ++row) {
        if (row < child_cs.n_rows) {
            selectors[kActive][row] =
                gf::Fp3::One();
        } else {
            selectors[kPadding][row] =
                gf::Fp3::One();
        }
        if (row + 1 < child_cs.n_rows) {
    ...
    selectors[kFirst][0] = gf::Fp3::One();
    selectors[kLast][child_cs.n_rows - 1] =
        gf::Fp3::One();
    for (uint32_t selector = 0;
         selector < kSelectors; ++selector) {
        lifted.preprocessed.emplace_back(
            selector_base + selector,
            std::move(selectors[selector]));
    }
    ...
    for (uint32_t column = 0;
        ...
        wrap_constraints.push_back(
            {glue::FormulaV1::EqualCurrent,
             ...
             wrap_base + column,
             column, 0});
        wrap_names.push_back("lift.wrap.first");
        wrap_constraints.push_back(
            {glue::FormulaV1::CarryTransition,
             aq::AirKind::kTransition,
             wrap_base + column, 0, 0});
    ...
    constraint_bytecode::ProgramTable wrap_program;
    if (!glue::BuildCanonicalProgramTableV1(
            lifted.n_columns,
            wrap_constraints,
            wrap_program, why) ||
    ...
    if (!constraint_bytecode::
            AppendLiftedAirConstraintsV1(
                ...
                lift_report, why)) {
        return Fail(
            why, "public_lift_child_bytecode");
    }
    ...
    if (!glue::BuildCanonicalProgramTableV1(
        ...
        !glue::AppendCanonicalConstraintsV1(
            ...
            nullptr, why)) {
        return Fail(
            why, "public_lift_padding_bytecode");
    }

    ChildAttachmentV1 attached;
    if (!AppendChildConstraintSystemV1(
            parent_cs, lifted,
            child_ordinal, attached, why)) {
        return false;
    }
