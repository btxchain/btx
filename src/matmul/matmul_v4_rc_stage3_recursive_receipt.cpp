    ...
    bool prove)
{
    ...
    // The L2 verifier consumes each decoded proof under its child CS and
    // seed, but the ordered envelope root is not yet a public parent AIR
    // cell. The separate receipt-owned local-q sum proof must also become a
    // recursively consumed child before CompleteFP can close.
    out.ordered_receipt_root_parent_air_bound = false;
        ...
        out.valid
            ? (std::string(
               ...
               (prove
                    ...
                    : ";parent_prove=0") +
               ";parent_receipt_root_pin=0")
            : "stage3:recursive_receipt:"
              "l2_consume_failed:" + out.l2.note;
    return out;
}
    ...
    out.note =
        out.valid
            ? (std::string(
               ...
               "recursive_consumption=0")
            : "stage3:recursive_receipt:"
              "q_join_failed:" + out.air_join.note;
    return out;
}
