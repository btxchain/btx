...
public:
     ...
     */
    explicit SpanReader(Span<const unsigned char> data)
        : m_start{data.data()}, m_total_size{data.size()}, m_data{data} {}
    ...
    size_t GetPos() const { return m_total_size - m_data.size(); }
    bool SetPos(size_t n)
    {
        if (n > m_total_size) {
            return false;
        }
        m_data = Span<const unsigned char>{m_start + n, m_total_size - n};
        return true;
    }
...
public:
    ...
    //! return the number of bytes remaining until the read limit
    uint64_t size() const {
        return nReadLimit - m_read_pos;
    }
