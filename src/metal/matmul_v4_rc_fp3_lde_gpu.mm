...
std::vector<std::string> Fp3LdeMetallibCandidatePaths()
{
    std::vector<std::string> paths;
    AppendUniquePath(paths, std::getenv("BTX_RC_FP3_LDE_METALLIB_PATH"));
    if (const auto executable_dir = ExecutableDirectory()) {
        AppendUniquePath(paths, (*executable_dir + "/metal/matmul_v4_rc_fp3_lde_gpu.metallib").c_str());
    }
#if defined(BTX_RC_FP3_LDE_METALLIB_PATH)
    AppendUniquePath(paths, BTX_RC_FP3_LDE_METALLIB_PATH);
#endif
    return paths;
}
...
struct MetalFp3LdeRuntime {
    ...
    MetalFp3LdeRuntime()
    {
        @autoreleasepool {
            ...
            for (const auto& candidate : Fp3LdeMetallibCandidatePaths()) {
