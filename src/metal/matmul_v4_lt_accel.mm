                      ...
                      uint2 tgid [[threadgroup_position_in_grid]])
{
    ...
    threadgroup signed char tile_a[TILE][TILE];
    threadgroup signed char tile_b[TILE][TILE];
    const int col = int(tgid.x * TILE + tid.x);
    const int row = int(tgid.y * TILE + tid.y);
    int acc = 0;

    for (int k0 = 0; k0 < K; k0 += int(TILE)) {
        const int ak = k0 + int(tid.x);
        const int bk = k0 + int(tid.y);
        tile_a[tid.y][tid.x] =
            (row < M && ak < K)
                ? A[size_t(row) * size_t(K) + size_t(ak)]
                : signed char(0);
        tile_b[tid.y][tid.x] =
            (bk < K && col < N)
                ? B[size_t(bk) * size_t(N) + size_t(col)]
                : signed char(0);
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint t = 0; t < TILE; ++t) {
            // signed char: never promote 0xFF as +255 (unsigned-char trap).
            acc += int(tile_a[tid.y][t]) * int(tile_b[t][tid.x]);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
    if (row < M && col < N) {
                       ...
                       uint32_t rows, uint32_t k, uint32_t cols, std::vector<int32_t>& out)
{
    ...
    constexpr NSUInteger kTile = 16;
    const MTLSize groups =
        MTLSizeMake((cols + kTile - 1) / kTile, (rows + kTile - 1) / kTile, 1);
    const MTLSize tgroup = MTLSizeMake(kTile, kTile, 1);
    [enc dispatchThreadgroups:groups threadsPerThreadgroup:tgroup];
    [enc endEncoding];
    [cmd commit];
