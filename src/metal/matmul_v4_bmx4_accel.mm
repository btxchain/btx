...
std::vector<std::string> MatMulBmx4Metal4MetallibCandidatePaths()
{
    std::vector<std::string> paths;
    AppendUniquePath(paths, std::getenv("BTX_MATMUL_V4_BMX4_METAL4_METALLIB_PATH"));

    if (const auto executable_dir = ExecutableDirectory()) {
        const std::string packaged_path = *executable_dir + "/metal/matmul_v4_bmx4_accel_kernels_metal4.metallib";
        AppendUniquePath(paths, packaged_path.c_str());
    }

#if defined(BTX_MATMUL_V4_BMX4_METAL4_METALLIB_PATH)
    AppendUniquePath(paths, BTX_MATMUL_V4_BMX4_METAL4_METALLIB_PATH);
#endif
    return paths;
}
...
struct MetalBmx4Context {
    ...
    void InitTensorPipeline()
    {
        ...
        // BTX_MATMUL_V4_BMX4_TENSOR_OPS=0 forces the ALU path.
        ...
        for (const auto& candidate_path : MatMulBmx4Metal4MetallibCandidatePaths()) {
            NSString* precompiled_path = [NSString stringWithUTF8String:candidate_path.c_str()];
            if (![[NSFileManager defaultManager] fileExistsAtPath:precompiled_path]) {
                continue;
            }
        ...
        if (pipeline == nil && tensor_path_reason == "ok:precompiled_metallib") {
            ...
            if (tensor_library != nil) {
                pipeline = MakePipeline(tensor_library, @"matmul_v4_bmx4_s8_gemm_s32_tensor", pipeline_error);
