...
This is the crypto core behind the btx.dev in-browser wallet generator. Native
node recovery for browser `.btxwallet` exports is documented in
`doc/btxwallet-browser-node-interop.md` and implemented by the
...
./build-wasm.sh            # -> dist/btx-pqc.js + dist/btx-pqc.mjs + dist/btx-pqc.wasm
```
...
| `dist/btx-pqc.mjs` | Same loader with a Node-friendly ES module extension |
...
`.btxwallet` JSON bundle. The node recomputes and verifies those descriptors
...
```js
import createBtxPqcModule from './btx-pqc.mjs';
