...
{
    // The old index used overflowing CAmount counters and must not be opened
    // using the new serialization format. Keep it intact for downgrades and
    // build the corrected index in a versioned path.
    const fs::path old_path{gArgs.GetDataDirNet() / "indexes" / "coinstats"};
    if (fs::exists(old_path)) {
        LogWarning("Old coinstatsindex found at %s. It can be safely deleted unless you plan to downgrade.\n",
...
bool CoinStatsIndex::ReverseBlock(const CBlock& block, const CBlockIndex* pindex)
{
    ...
    // Roll back MuHash by removing the new UTXOs created by the block and
    // reapplying the old UTXOs spent by the block. Other cumulative values
    // are restored from the preceding index entry below.
    for (size_t i = 0; i < block.vtx.size(); ++i) {
        const auto& tx{block.vtx.at(i)};
        const bool is_coinbase{tx->IsCoinBase()};

        if (is_coinbase && IsBIP30Unspendable(*pindex)) {
            continue;
        }
        ...
        for (uint32_t j = 0; j < tx->vout.size(); ++j) {
            const CTxOut& out{tx->vout[j]};
            COutPoint outpoint{tx->GetHash(), j};
            Coin coin{out, pindex->nHeight, is_coinbase};
    ...
    // Restore the overflow-safe counters and the remaining aggregate values
    // from the preceding block's index entry.
