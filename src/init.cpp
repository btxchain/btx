...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-matmulattestationserve", "Serve GETMMATTEST from the local attestation store (default: 1 when a local signing key is configured or when -matmulvalidation=trusted, otherwise 0). Trusted mirrors cache-and-forward signatures they have already accepted; they never SignAuthoritative. A local signer serves cached statements at any canonical height but limits uncached regeneration/reverify to the 16-block live tip window for public peers. Consensus signers may set this to 0 to isolate signing from public GETMMATTEST fan-in; newly signed attestations are still pushed to connected peers. Aggressive public GETMMATTEST / MMATTEST (rate-limit exhaustion or historical scans of a signer) is penalized: the peer is disconnected and banned for 24h. When no signature is cached, a serving consensus signer with a local ExactReplay-success bit may regenerate its own statement; otherwise a rate-limited background ExactReplay may be queued for canonical Profile-1 blocks.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-matmulattestationbackfillwindow=<n>", strprintf("Allow peers explicitly granted the matmulbackfill P2P permission to request uncached canonical MatMul attestations up to n blocks behind a local signer's tip (default/public window: %d, maximum: %d). Cached statements remain available outside this window. Grant narrowly with -whitelist=in,matmulbackfill@<subnet>, -whitelist=out,matmulbackfill@<subnet>, or both directions; addnode/noban/download alone do not grant historical replay authority.", node::matmul_trusted::SIGNER_GETMMATTEST_SERVE_WINDOW, node::matmul_trusted::SIGNER_GETMMATTEST_BACKFILL_WINDOW_MAX), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
