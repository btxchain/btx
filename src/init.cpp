...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-signermintargetcompact=<hex>", "LOCAL POLICY, NOT CONSENSUS. Compact nBits pin for extra MatMul digest work on this node only. Header nBits stay on GetNextWorkRequired (archives would reject a harder nBits as bad-diffbits). When set, this node refuses blocks whose matmul_digest is easier than the pin. Requires -signerextraworkfromheight. Unset = disabled.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-signerextraworkfromheight=<n>", "LOCAL POLICY, NOT CONSENSUS. First height at which -signermintargetcompact is enforced. Must be set with the compact pin so already-stored easier blocks remain loadable on restart/reindex. Must be >= 0.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
