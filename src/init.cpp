...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-allowunpinnedshieldedsnapshot", "Allow loadtxoutset to load an assumeutxo snapshot whose shielded section (pool balance + nullifier set + commitment tree) has no consensus pin for its height (default: 0). Off by default (fail-closed): an unpinned shielded section is attacker-supplied and not validated against consensus, so loading it can seed a double-spend. Set to 1 only to bootstrap from a snapshot you trust out-of-band before its pin is filled in.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
