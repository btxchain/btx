...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-matmulattestationserve", "Serve GETMMATTEST from the local attestation store (default: 1 when a local signing key is configured or when -matmulvalidation=trusted, otherwise 0). Trusted mirrors cache-and-forward signatures they have already accepted; they never SignAuthoritative. A local signer serves only the live tip window; historical GETMMATTEST is ignored. Consensus signers may set this to 0 to isolate signing from public GETMMATTEST fan-in; newly signed attestations are still pushed to connected peers. Aggressive GETMMATTEST / MMATTEST (rate-limit exhaustion or historical scans of a signer) is penalized: the peer is disconnected and banned for 24h. When no signature is cached, a serving consensus signer with a local ExactReplay-success bit may regenerate its own statement; otherwise a rate-limited background ExactReplay may be queued for canonical Profile-1 blocks.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
