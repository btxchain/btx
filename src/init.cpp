...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-reorgprotectionprofile=<mode>", "Per-node reorg/finality profile: emergency (default), strict, balanced, or archive. emergency/strict warn after 3-block rewrites and park after 12; balanced warns after 12 and parks after 48; archive follows most work and only alarms. This is local policy, not consensus finality.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-parkdeepreorg", "Per-node deep-reorg parking action override (NON-CONSENSUS). When 1, the node refuses to auto-switch to a branch deeper than -maxreorgdepthpark/profile park depth and stays on its current tip pending operator action. When 0, it follows most work and only alarms. (default: profile-dependent, emergency=1)", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-maxreorgdepthwarn=<n>", "Warn/alarm when a candidate branch would reorganize more than this many blocks (default: active -reorgprotectionprofile warn depth, emergency=3). Must be >= 1.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-maxreorgdepthpark=<n>", "Park/refuse automatic switch when a candidate branch would reorganize more than this many blocks and -parkdeepreorg=1 (default: active -reorgprotectionprofile park depth, emergency=12). Must be >= 1.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    argsman.AddArg("-localfinalitydepth=<n>", "Report practical local finality after this many confirmations in mining/difficulty RPCs (default: active -reorgprotectionprofile finality depth, emergency=12). This is not consensus finality. Must be >= 1.", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-miningchainguardminneartippeers=<n>", strprintf("Minimum outbound peers that must be within -miningchainguardneartipwindow blocks of the local tip before local mining continues (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_MIN_NEAR_TIP_PEERS), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    ...
    argsman.AddArg("-miningchainguardneartipwindow=<n>", strprintf("Block window used to count near-tip outbound peers for local mining chain guard (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_NEAR_TIP_WINDOW), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
    argsman.AddArg("-miningchainguardstalepeerseconds=<n>", strprintf("Seconds after which lagging peers without fresh block signals are ignored by local mining chain guard (default: %d)", node::DEFAULT_MINING_CHAIN_GUARD_STALE_PEER_SECONDS), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
