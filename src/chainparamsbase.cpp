...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-regtestrcunifiedheight=<n>", "v4.6 single-switch: set ENC_RC episode + ENC_RC_COUPLED (V3 production) to the SAME activation height so the whole RC family transitions at once with no staggered regime. Equivalent to -regtestrcheight=n -regtestrccoupledheight=n; a later -regtestrcheight/-regtestrccoupledheight still refines one component (regtest-only; public nets stay INT32_MAX)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    ...
    argsman.AddArg("-regtestrcprofile=<1|2>", "Select ENC_RC episode dimension profile on regtest: 1=epoch-0 base (default), 2=datacenter (rounds=8, L=64, b_seq=32768; ~16x per-episode MAC). Pair with a finite -regtestrcheight (and -regtestrctoydims for CI-scale mining) to activate the datacenter episode. Invalid values (!={1,2}) fail closed at construction (regtest-only; public nets default to profile 1)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
