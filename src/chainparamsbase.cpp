...
void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-regtestshieldedpooldisableheight=<n>", "Override regtest shielded pool disable height (regtest-only). At/after this height shielded ingress and egress are consensus-invalid and shielded state is not opened unless -shieldedstate=1.", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    ...
    argsman.AddArg("-regtestcontenteliminationheight=<n>", "Override regtest content-elimination hard-fork height (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    ...
    argsman.AddArg("-regtestdrltheight=<n>", "Override regtest MatMul v4.4-LT Rank-1 ENC-DR-LT encoding-profile activation height; must be >= the (possibly also-overridden) ENC-BMX4C height (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrcheight=<n>", "Override regtest MatMul ENC_RC (Resident Curriculum) activation height; must be >= the (possibly also-overridden) MatMul v4 height (regtest-only; public-net heights are fixed in chainparams, with testnet/signet disabled)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrccoupledheight=<n>", "Override regtest MatMul ENC_RC_COUPLED activation height; must be >= the (possibly also-overridden) MatMul v4 height and, when ENC_RC is also live, >= -regtestrcheight (regtest-only; public nets keep the coupled height at INT32_MAX)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrcunifiedheight=<n>", "Legacy PR89 regression switch: set ENC_RC episode + ENC_RC_COUPLED to the SAME regtest height. MatMul v4.7 does not use this as a public activation model; its epochs require separate reviewed heights. Equivalent to -regtestrcheight=n -regtestrccoupledheight=n; a later granular override may refine one component (regtest-only; public nets never enable the coupled profile)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrctoydims=<0|1>", "Use CI-scale toy ENC_RC episode dims on regtest when ENC_RC is live (1) instead of consensus production dims (0); required for practical mining when -regtestrcheight is finite (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrccoupledtoydims=<0|1>", "Use CI-scale toy ENC_RC_COUPLED dims on regtest when the coupled profile is live (1) instead of medium/production dims (0). Cross with -regtestrccoupledprofile: 2×1=toy-V2, 2×0=medium-V2, 3×1=medium-V3-CI, 3×0=production-V3 (regtest-only)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrccoupledprofile=<2|3>", "Select ENC_RC_COUPLED consensus shape family on regtest: 2=V2 (default), 3=V3. Invalid values fail closed (ResolveRCCoupParams → zero params / reject). Pair with -regtestrccoupledtoydims (regtest-only; public nets default to profile 3 = V3 production)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrcprofile=<1|2>", "Select the ENC_RC episode profile on regtest: 1=MatMul v4.7 Epoch-A base dimensions and ExactReplay authority (default), 2=future proof-authoritative datacenter dimensions (rounds=8, L=24, b_seq=87552; ~16x per-episode MAC). Pair with a finite RC height (and -regtestrctoydims for CI-scale mining) to exercise activation. Invalid values (!={1,2}) fail closed at construction (regtest-only; public mainnet fixes Profile 1 and its release-selected height in chainparams)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestmatmulflatsketchreplay", "Re-select the legacy FLAT_SKETCH_INBLOCK MatMul v4 carriage instead of the v4.4 ENC-DR digest-only carriage (regtest-only; golden-vector replay / differential testing)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestmatmulltsealaspow=<0|1>", "Select the MatMul v4.4-LT lottery object on regtest: Q* Phase B window seal (1, default) or Phase-A per-nonce digest (0); requires a live -regtestdrltheight when enabled", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestmatmulltmaxpending=<n>", "Override the MatMul v4.4-LT pending verification job cap on regtest (regtest-only admission-boundary testing)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestmatmulmaxpending=<n>", "Override the EncDr pending verification job cap (nMatMulMaxPendingVerifications) on regtest (regtest-only admission-boundary testing)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestrcmaxpending=<n>", "Override the Profile-1 RC pending verification job cap (nMatMulRCMaxPendingVerifications) on regtest. Applied after the default RC unthrottle so a functional test can still exhaust the cap (regtest-only admission-boundary testing)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestmatmulproofprunedepth=<n>", "LEGACY (inert since v4.4 ENC-DR): override the regtest MatMul proof-prune window in blocks. The segregated-proof store was deleted; no code consumes this value (regtest-only, retained for arg compatibility)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
    argsman.AddArg("-regtestmatmulproofassumevalidminage=<n>", "Override the regtest MatMul v4.4 ENC-DR assumevalid buried-RECOMPUTE trust minimum equivalent-time age in seconds (regtest-only; production default is the 2-week DoS guard). Below the assumevalid block, blocks older than this skip the digest recompute", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::CHAINPARAMS);
