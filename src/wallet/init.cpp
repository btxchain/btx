...
void WalletInit::AddWalletOptions(ArgsManager& argsman) const
{
    ...
    argsman.AddArg("-walletreorgsafetydepth=<n>", strprintf("Minimum confirmations wallet RPCs report as settlement-safe for reorg-aware services (default: %u). This does not change consensus validity or legacy trusted-balance semantics.", DEFAULT_WALLET_REORG_SAFETY_DEPTH), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    argsman.AddArg("-walletreorgholdblocks=<n>", strprintf("After any block disconnect/reorg, report wallet settlement_safe=false and settlement-safe balances as zero until this many additional blocks have built after the observed reorg hold anchor (default: %u, 0 disables the block component). This is wallet/service policy and does not change consensus validity or legacy trusted-balance semantics.", DEFAULT_WALLET_REORG_HOLD_BLOCKS), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    argsman.AddArg("-walletreorgholdseconds=<n>", strprintf("After any block disconnect/reorg, keep wallet settlement_safe reporting disabled for at least this many seconds after the reorg is observed (default: %u, 0 disables the time component). This prevents a premined replacement branch from burning down the hold before services can react.", DEFAULT_WALLET_REORG_HOLD_SECONDS), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
