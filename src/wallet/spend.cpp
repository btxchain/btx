...
void CoinsResult::Erase(const std::unordered_set<COutPoint, SaltedOutpointHasher>& coins_to_remove)
{
    for (auto& [type, vec] : coins) {
        auto remove_it = std::remove_if(vec.begin(), vec.end(), [&](const COutput& coin) {
            ...
            if (coin.HasEffectiveValue() && total_effective_amount.has_value()) {
...
CoinsResult AvailableCoinsListUnspent(const CWallet& wallet, const CCoinControl* coinControl, CoinFilterParams params)
{
    ...
    return *Assert(AvailableCoins(wallet, coinControl, /*feerate=*/ std::nullopt, params));
}
...
util::Result<SelectionResult> ChooseSelectionResult(interfaces::Chain& chain, const CAmount& nTargetValue, Groups& groups, const CoinSelectionParams& coin_selection_params)
{
    ...
    for (auto& result : results) {
        ...
        // The mempool can change between the individual and combined bump-fee
        // snapshots. Never pass a negative discount to SelectionResult.
