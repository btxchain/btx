...
namespace
{
/// Internal SHA-384 implementation.
namespace sha384
{
...
{
    s[0] = 0xcbbb9d5dc1059ed8ull;
    s[1] = 0x629a292a367cd507ull;
    s[2] = 0x9159015a3070dd17ull;
    s[3] = 0x152fecd8f70e5939ull;
    s[4] = 0x67332667ffc00b31ull;
    s[5] = 0x8eb44a8768581511ull;
    s[6] = 0xdb0c2e0d64f98fa7ull;
    s[7] = 0x47b5481dbefa4fa4ull;
}

/** Perform one SHA-384 transformation, processing a 128-byte chunk. */
...
CSHA384& CSHA384::Write(const unsigned char* data, size_t len)
{
    ...
    if (bufsize && bufsize + len >= 128) {
        ...
        bytes += 128 - bufsize;
        data += 128 - bufsize;
        sha384::Transform(s, buf);
        bufsize = 0;
    }
    while (end - data >= 128) {
        // Process full chunks directly from the source.
        sha384::Transform(s, data);
        data += 128;
        bytes += 128;
    }
    if (end > data) {
...
void CSHA384::Finalize(unsigned char hash[OUTPUT_SIZE])
{
    ...
    WriteBE64(tmp, s[0]);
    WriteBE64(tmp + 8, s[1]);
    WriteBE64(tmp + 16, s[2]);
    WriteBE64(tmp + 24, s[3]);
    WriteBE64(tmp + 32, s[4]);
    WriteBE64(tmp + 40, s[5]);
    WriteBE64(tmp + 48, s[6]);
    WriteBE64(tmp + 56, s[7]);
    memcpy(hash, tmp, CSHA384::OUTPUT_SIZE);
}

CSHA384& CSHA384::Reset()
{
    bytes = 0;
    sha384::Initialize(s);
    return *this;
