                               ...
                               int32_t block_height, bool* carrier_missing)
{
    ...
    if (params.nMatMulRCProfile == 2) {
        ...
        // the complete transcript-version + 9-field episode shape — IDENTICAL to
        // the fields the FS seed binds in RCGkrFsSeedV7. d_ff is load-bearing: it
        // is the fused-FFN inner width and the dominant compute lever (FFN MAC =
        // 2·b_seq·d_model·d_ff·L·rounds, compute/hash margin = 2·d_ff), so
        // omitting it would let a miner declare a tiny d_ff and pass an
        // honest-but-cheap episode. phase1_tile_delta is an RCEpisodeOptions
        // execution knob, not a shape field.
        const auto& e = dc_carrier.episode;
        if (!(e.transcript_version == params_rc.transcript_version &&
              e.rounds == params_rc.rounds && e.d_head == params_rc.d_head &&
