...
class BTXMatMulRCActivation(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # Keep the offline node in real IBD from its first startup. Otherwise
        # regtest can permanently latch IBD complete at genesis before the
        # source node has mined the chain this node is intended to download.
        self.extra_args = [common, common, [*common, "-maxtipage=1"]]
    ...
    def run_test(self):
        ...
        work_profile = challenge["work_profile"]
        assert_equal(work_profile["profile_kind"], "matmul_v47_rc")
        assert_equal(work_profile["b"], challenge["matmul"]["b"])
        assert "sketch_payload_bytes" not in work_profile
        assert "sketch_dimension" not in work_profile
        assert work_profile["rc_episode"]["rounds"] > 0
        assert work_profile["rc_episode"]["total_macs"] > 0
        ...
        work_profile = challenge["work_profile"]
        assert_equal(
            work_profile["profile_kind"],
            "matmul_v47_rc_coupled",
        )
        assert_equal(work_profile["b"], challenge["matmul"]["b"])
        assert "sketch_payload_bytes" not in work_profile
        assert "sketch_dimension" not in work_profile
        assert work_profile["rc_coupled"]["barriers"] > 0
        assert work_profile["rc_coupled"]["total_macs"] > 0
