...
def deterministic_matmul_seed_v2(header, height, which):
    payload = b""
    payload += ser_string(b"BTX_MATMUL_SEED_V2")
...
class msg_getmmattest:
    """Request retained trusted attestations for one block hash."""
    __slots__ = ("block_hash",)
...
class msg_mmattest:
    """Opaque trusted-attestation vector used by generic functional peers."""
    __slots__ = ("data",)
    msgtype = b"mmattest"
    ...
    def __repr__(self):
        return "msg_mmattest(data_len=%d)" % len(self.data)
