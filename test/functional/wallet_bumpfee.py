...
class BumpFeeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Automatic wallet creation is intentionally P2MR-only. The generic
        # bumpfee paths above exercise native P2MR inputs; the Bitcoin-specific
        # SegWit source fixture cannot be generated by this wallet policy.
        ...
        test_bumpfee_with_descendant_fails(self, rbf_node, peer_node, rbf_node_address, dest_address)
    ...
    def test_single_output(self):
        ...
        addr = wallet.getnewaddress()
        # Native P2MR inputs have large witnesses. Fund enough value that the
        # later 20 sat/vB two-input replacement can pay its fee and retain an
        # economical change output; 0.001 BTX is insufficient for that shape.
        amount = Decimal("0.01")
        # Make 2 UTXOs
...
def test_notmine_bumpfee(self, rbf_node, peer_node, dest_address):
    ...
    # Keep the original fee above relay minimum but low enough that one of the
    # wallet's 0.001 BTX coins can fund a default BIP125 replacement after its
    # P2MR input cost is accounted for.
    fee = Decimal("0.0002")
    rbf_utxo = next(utxo for utxo in rbf_node.listunspent() if utxo["amount"] == Decimal("0.001"))
    peer_utxo = max(
        (utxo for utxo in peer_node.listunspent(minimumAmount=fee) if utxo["scriptPubKey"].startswith("5220")),
        key=lambda utxo: utxo["amount"],
    )
    utxos = [rbf_utxo, peer_utxo]
    inputs = [{
        "txid": utxo["txid"],
        "vout": utxo["vout"],
    ...
    def finish_psbtbumpfee(psbt):
        ...
        finalized = rbf_node.finalizepsbt(psbt["psbt"])
        assert finalized["complete"], {"processed": psbt, "finalized": finalized}
        res = rbf_node.testmempoolaccept([finalized["hex"]])
    ...
    # Replacing the recipient set creates explicit fee headroom without adding another
    # large P2MR input; the PSBT must still be completed by both input owners.
    replacement_outputs = [{dest_address: output_val - Decimal("0.01")}]
    psbt = rbf_node.psbtbumpfee(txid=rbfid, fee_rate=old_feerate + 10, outputs=replacement_outputs)
    finish_psbtbumpfee(psbt["psbt"])

    self.clear_mempool()


def test_bumpfee_with_descendant_fails(self, rbf_node, peer_node, rbf_node_address, dest_address):
    ...
    # Create a native P2MR descendant that is present in the mempool but not in
    # the RBF wallet. Bitcoin's MiniWallet emits a P2WPKH output, which is not a
    # standard destination under BTX's P2MR-only wallet policy.
    peer_address = peer_node.getnewaddress()
    parent_id = spend_one_input(rbf_node, peer_address)
    self.sync_mempools()
    parent_vout = find_vout_for_address(peer_node, parent_id, peer_address)
    peer_node.send(
        outputs={peer_node.getnewaddress(): Decimal("0.0002")},
        options={"inputs": [{"txid": parent_id, "vout": parent_vout}], "fee_rate": ECONOMICAL},
    )
    self.sync_mempools()
...
def test_bumpfee_with_abandoned_descendant_succeeds(self, rbf_node, rbf_node_address, dest_address):
    ...
    # parent is send-to-self, so we don't have to check which output is change when creating the child tx
    # Give the native P2MR parent a feerate above the restart floor while the
    # child remains below it. The generic fixture's smaller fee would evict
    # both transactions because P2MR inputs are much larger than P2WPKH.
    parent_id = spend_one_input(rbf_node, rbf_node_address, change_size=Decimal("0.00036000"))
    ...
    self.restart_node(1, ['-minrelaytxfee=0.00003'] + self.extra_args[1])
...
def test_dust_to_fee(self, rbf_node, dest_address):
    ...
    # Select the feerate from the native transaction's measured size. A target
    # fee just below the 0.0005 BTX non-payment value leaves only dust change;
    # dropping that output then contributes the full remainder to the fee.
    target_fee_sats = Decimal("0.00049") * COIN
    dust_fee_rate = (target_fee_sats / fulltx["vsize"]).quantize(Decimal("0.001"), rounding=ROUND_UP)
    bumped_tx = rbf_node.bumpfee(rbfid, fee_rate=dust_fee_rate)
...
def test_maxtxfee_fails(self, rbf_node, dest_address):
    ...
    # A native P2MR replacement at the wallet's selected fee exceeds the
    # deliberately tiny maximum transaction fee and must be rejected.
    self.restart_node(1, ['-maxtxfee=0.000025'] + self.extra_args[1])
    rbf_node.walletpassphrase(WALLET_PASSPHRASE, WALLET_PASSPHRASE_TIMEOUT)
    rbfid = spend_one_input(rbf_node, dest_address)
...
def test_watchonly_psbt(self, peer_node, rbf_node, dest_address):
    ...
    # Public pqhd() descriptors intentionally cannot derive PQ keys without the
    # seed. Let a normal native wallet derive the keys, then give the watcher
    # fixed-key forms of the same P2MR descriptors. The signer retains the
    # corresponding PQHD private keys and can complete either PSBT input.
    rbf_node.createwallet(wallet_name="signer", disable_private_keys=False, blank=False, descriptors=True)
    signer = rbf_node.get_wallet_rpc("signer")
    assert signer.getwalletinfo()['private_keys_enabled']

    funding_addresses = [signer.getnewaddress(address_type="p2mr") for _ in range(2)]
    ...
    def fixed_p2mr_descriptor(address):
        ...
        descriptor = rbf_node.getdescriptorinfo(f"mr({mldsa_key},{slhdsa_key})")["descriptor"]
        assert_equal(rbf_node.deriveaddresses(descriptor)[0], address)
        return descriptor

    funding_descriptors = [fixed_p2mr_descriptor(address) for address in funding_addresses]
    change_descriptor = fixed_p2mr_descriptor(change_address)

    # Create another wallet with just the public keys, which creates PSBTs
    rbf_node.createwallet(wallet_name="watcher", disable_private_keys=True, blank=True, descriptors=True)
    ...
    reqs = [{
        ...
        "timestamp": "now",
        "internal": False,
    } for descriptor in funding_descriptors]
    reqs.append({
    ...
    assert_equal(result, [{'success': True}, {'success': True}, {'success': True}])

    peer_node.sendmany("", {address: Decimal("0.05") for address in funding_addresses})
    self.generate(peer_node, 1)
    assert_equal(len(watcher.listunspent()), 2)

    # Create single-input PSBT for transaction to be bumped
    # Leave enough change for the original transaction, but not enough for the
    # high-feerate replacement so psbtbumpfee must add the second watch-only input.
    psbt = watcher.walletcreatefundedpsbt([watcher.listunspent()[0]], {dest_address: Decimal("0.0495")}, 0,
            {"fee_rate": 1, "add_inputs": False, "change_address": change_address}, True)['psbt']
    ...
    # Bump high enough that reducing change is insufficient and a second input is required.
    watchonly_add_input_fee_rate = 10
    bumped_psbt = watcher.psbtbumpfee(original_txid, fee_rate=watchonly_add_input_fee_rate)
...
def test_rebumping(self, rbf_node, dest_address):
    ...
    rbfid = spend_one_input(rbf_node, dest_address)
    # Native P2MR inputs are large enough that rounding the original fee to an
    # integer sat/vB can leave a 2 sat/vB replacement just below the absolute
    # incremental-relay-fee delta. Use two unambiguously increasing rates so
    # this test continues to exercise replacement ancestry, not fee rounding.
    bumped = rbf_node.bumpfee(rbfid, fee_rate=NORMAL)
...
def test_rebumping_not_replaceable(self, rbf_node, dest_address):
    ...
    rbfid = spend_one_input(rbf_node, dest_address)
    bumped = rbf_node.bumpfee(rbfid, fee_rate=NORMAL, replaceable=False)
...
def test_change_script_match(self, rbf_node, dest_address):
    ...
    # Leave enough change that two native-P2MR replacements do not cross the
    # wallet's economical-change threshold and intentionally drop the output.
    rbfid = spend_one_input(
        rbf_node,
        dest_address,
        payment_size=Decimal("0.00020000"),
        change_size=Decimal("0.00076000"),
    )
    change_addresses = get_change_address(rbfid, rbf_node)
    assert_equal(len(change_addresses), 1)

    # Now find that address in each subsequent tx, and no other change
    bumped_total_tx = rbf_node.bumpfee(rbfid, fee_rate=NORMAL)
    assert_equal(change_addresses, get_change_address(bumped_total_tx['txid'], rbf_node))
    bumped_rate_tx = rbf_node.bumpfee(bumped_total_tx["txid"], fee_rate=HIGH)
...
def spend_one_input(
    node,
    dest_address,
    change_size=Decimal("0.00046000"),
    data=None,
    payment_size=Decimal("0.00050000"),
):
    tx_input = dict(
        sequence=MAX_BIP125_RBF_SEQUENCE, **next(u for u in node.listunspent() if u["amount"] == Decimal("0.00100000")))
    destinations = {dest_address: payment_size}
    if change_size > 0:
...
def test_feerate_checks_replaced_outputs(self, rbf_node, peer_node):
    ...
    # Calculate the minimum feerate required for the bump. Since all but one
    # output are replaced, subtract their actual serialized sizes rather than
    # assuming Bitcoin's 31-byte P2WPKH output.
    tx_size = tx_details["decoded"]["vsize"]
    serialized_output_sizes = []
    for txout in tx_details["decoded"]["vout"]:
        script = bytes.fromhex(txout["scriptPubKey"]["hex"])
        serialized_output_sizes.append(8 + len(ser_compact_size(len(script))) + len(script))
    est_bumped_size = tx_size - sum(serialized_output_sizes[1:])
