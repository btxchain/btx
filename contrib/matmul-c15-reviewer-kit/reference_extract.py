...
def verify_goldens(vectors: dict | None = None) -> int:
    ...
    expected_key = bytes.fromhex(v["expected_prf_digest_hex"])
    ok = True
    if prf_key != expected_key:
        print(f"FAIL prf_key: got {prf_key.hex()} expected {v['expected_prf_digest_hex']}")
        ok = False
    ...
    if "expected_chacha_cell_prf_digest_hex" in v and cell_key != bytes.fromhex(v["expected_chacha_cell_prf_digest_hex"]):
        print(
            f"FAIL cell_prf_key: got {cell_key.hex()} expected {v['expected_chacha_cell_prf_digest_hex']}"
        )
        ok = False
