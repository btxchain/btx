...
[[ -x "$BTXD" ]] || { echo "e2e-hosting-lifecycle: missing executable $BTXD (BIN=$BIN)" >&2; exit 1; }
[[ -x "$CLI" ]] || { echo "e2e-hosting-lifecycle: missing executable $CLI (BIN=$BIN)" >&2; exit 1; }
[[ -x "$MODELD" ]] || { echo "e2e-hosting-lifecycle: missing executable $MODELD (BIN=$BIN)" >&2; exit 1; }
# Dedicated ports: never share 18443/18444 with the lab /var/lib/btxd node.
PORT=37644
RPC=37643
cli() {
  "$CLI" -regtest -datadir="$1" -rpcport="$RPC" -rpcuser=u -rpcpassword=p "${@:2}"
}

PID=""
cleanup() {
  if [[ -n "${PID:-}" ]] && kill -0 "$PID" 2>/dev/null; then
    cli "${DATADIR:-$WORKDIR/x}" stop >/dev/null 2>&1 || true
    sleep 0.5
...
wait_rpc() {
  ...
  for i in $(seq 1 80); do
    if cli "$dd" getblockchaininfo >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.25
  done
...
mkdir -p "$DATADIR"
"$BTXD" -regtest -datadir="$DATADIR" -listen=0 -server=1 -port="$PORT" -rpcport="$RPC" \
  -rpcuser=u -rpcpassword=p \
  -modelhelper=/bin/false -modelbind=off -daemon=0 >"$WORKDIR/crash.log" 2>&1 &
PID=$!
wait_rpc "$DATADIR"
cli "$DATADIR" getblockchaininfo >/dev/null
info="$(cli "$DATADIR" getmodelnetworkinfo)"
echo "START-06 $info"
python3 -c '
import json,sys
...
' "$info"
cli "$DATADIR" stop >/dev/null 2>&1 || true
sleep 1
PID=""
echo "START-06 PASS"
