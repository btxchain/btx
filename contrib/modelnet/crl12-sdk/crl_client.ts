 ...
 *  No /rpc. No executeAllocation. automatic_spend_atoms stays 0.
 */
...
const BLOCKED = new Set(["executeAllocation", "submitFinanceIntent", "genericRpc", "/rpc"]);
function assertTypedPath(path: string): void {
...
export class CrlClient {
  ...
  constructor(base:string, operations:Operation[], private headers:HeaderFactory,
    ...
    for (const o of operations) {
      assertTypedPath(o.path);
      if (BLOCKED.has(o.operation_id)) throw new Error("SCOPE_DENIED");
    }
    this.base=base.replace(/\/$/,"");this.ops=new Map(operations.map(o=>[o.operation_id,o]));
  }
  executeAllocation(_id?: string): never {
    throw new Error("SCOPE_DENIED: CRL/1.2 has no executeAllocation route");
  }
  async call(id:string, options:{path?:Record<string,string>;query?:Record<string,string>;
    ...
    if (BLOCKED.has(id)) throw new Error("SCOPE_DENIED: not a CRL/1.2 typed route");
    const op=this.ops.get(id);if(!op)throw new Error("UNKNOWN_OPERATION");
    assertTypedPath(op.path);
    if (op.path.includes("/execute")) throw new Error("SCOPE_DENIED");
