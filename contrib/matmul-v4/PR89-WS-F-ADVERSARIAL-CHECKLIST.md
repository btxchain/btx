...
decision; unfinished proof arbiters are OFF. Mainnet Epoch A is a scheduled
release candidate; the first item below reflects the disabled state originally
audited.

## Consensus safety

- [ ] `nMatMulRCHeight` / `nMatMulRCCoupledHeight` still `INT32_MAX` on public params
  (candidate: mainnet `nMatMulRCHeight` is one release-selected `H_A`; the coupled height
