...
Usage (run on the non-GPU host):
  ...
  SIGNER_WIF_FILE=/tmp/btx-trusted-signer.wif \\
  SIGNER_PUB_FILE=/tmp/btx-trusted-signer.pub \\
...
ARCHIVE_DD = "/tmp/btx-two-node-archive"
MIRROR_DD = Path("/tmp/btx-two-node-mirror")
ARCHIVE_RPC = 19821
ARCHIVE_P2P = 19822
MIRROR_RPC = 19831
MIRROR_P2P = 19832
OUT = Path("/tmp/btx-two-node-trusted-mirror-result.json")
...
"""
    ...
    sh_remote(
        ...
        f"-conf={ARCHIVE_DD}/btx.conf >/tmp/btx-two-node-archive.stdout 2>&1 & echo $!"
    )
