#!/usr/bin/env bash
#
# ENC_RC v4.6 (Resident Curriculum) — historical measurement entrypoint.
#
# MatMul v4.7 Epoch A uses Profile 1 ExactReplay. Profile-2 carrier/coupled
# results from this script are proof-development evidence only and have no
# consensus authority. See doc/btx-matmul-v4.7-transition-roadmap.md.
# The legacy
...
#
# Every v4.7 epoch height stays disabled. Toy/PARTIAL never raises activation.
#
set -euo pipefail
...
case "$cmd" in
  ...
  verify-carrier)
    ...
    echo "All MatMul v4.7 epoch heights remain disabled (never raise from this bench alone)."
    exit "$CODE"
    ;;
  cuda-episode-tests)
    BIN=""
