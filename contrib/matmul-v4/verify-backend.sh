...
#
# Activation gate (see ACTIVATION.md): backend PASS results are necessary
# inputs to the full B1-B5 gate; no pair of hardware runs is sufficient by
# itself to authorize a mainnet hard fork.
#
# Usage:
...
#   contrib/matmul-v4/verify-backend.sh hip     # on an AMD CDNA host
#
...
case "$BACKEND" in
  ...
  cuda)  BACKEND_LABEL="CUDA"; CMAKE_FLAGS=(-DBTX_ENABLE_CUDA_EXPERIMENTAL=ON "-DBTX_CUDA_ARCHITECTURES=${CUDA_ARCH:-75;80;89;90;100;120}") ;;
  metal) BACKEND_LABEL="Metal"; CMAKE_FLAGS=(-DBTX_ENABLE_METAL=ON) ;;   # Apple; needs Xcode 26+ for the M5 tensor path
  hip)   BACKEND_LABEL="HIP/ROCm"; CMAKE_FLAGS=(-DBTX_ENABLE_HIP=ON "-DBTX_HIP_ARCHITECTURES=${HIP_ARCH:?set HIP_ARCH e.g. gfx942}") ;;
...
# AND the SELECTED DEVICE emitted the C-1 high-magnitude runtime marker. Boost's
# process status is authoritative for hard test failures. Do not grep generic
# words such as "CONSENSUS" or "SPLIT" here: successful Boost logs include
# unrelated suite names such as pq_consensus_tests, which would turn a clean
# backend run into a false digest-divergence failure.
if [ "$CODE" -ne 0 ]; then
...
# Other backend rows are expected to skip in a single-backend build. Only a
# skip naming the backend selected for this invocation makes this run
# inconclusive.
if echo "$OUT" | grep -qiE "SKIPPED-PENDING-HARDWARE: (v4 backend '${BACKEND}'|${BACKEND_LABEL} v4 determinism row)"; then
...
MARKER="$(echo "$OUT" | grep -oE "DEVICE_HIGH_MAGNITUDE_PASS:${BACKEND}:[^[:space:]]+" | head -1 || true)"
if [ -z "$MARKER" ]; then
...
echo "Record this result in ACTIVATION.md. Activation GO requires every B1-B5 gate."
exit 0
