...
| 2 | Strassen / Winograd / bilinear recursion | **BOUNDED** | Exactly **one** recursion level on INT8-path hardware (⌊log₂(127/48)⌋ = 1); **zero** levels on the FP4/FP8-exact frontier path (level-1 operand sums are not E2M1/E4M3-exact); mantissa-plane deep recursion blocked by per-(row,block) scales (≥ 5× loss, §3.3). MAC saving ≤ 12.5 % theoretical, class-restricted, difficulty-absorbed |
...
`Â[i,k] = μ_A[i,k]·2^(e_A(i,⌊k/32⌋))`, `μ ∈ 𝓜₁₁`, `e ∈ [0,3]`; sketch
...
### 3.3 The adversarial mantissa-plane variant (new, checked, closed)

Because mantissas alone are ≤ 6, mantissa-plane Strassen could in principle recurse to
6·2^d ≤ 127 → **d = 4** (0.586× multiplies). Can a miner factor the scales out to get a
pure-mantissa GEMM? No: scales are per-(row, k-block) on A and per-(k-block, column) on
B, so the product is `C = Σ_b diag(2^{e_A(·,b)}) · (μ_A^b·μ_B^b) · diag(2^{e_B(b,·)})`
— the mantissa GEMMs are n×32×n slabs pinned between block-dependent diagonals, and
K = 32 slabs admit no useful Strassen recursion (the multiply-saving recursion needs
all three dimensions to split). The alternative — masking by scale class,
`Â = Σ_{e=0..3} 2^e·Â_e` — gives 16 mantissa GEMM pairs (4×4 scale classes), each ~25 %
dense: 16·(7/8)⁴ = **9.4× more multiplies than the direct evaluation**; even granting
a free idealized 2× from structured-sparsity units (which random 75 % sparsity does not
legally fit — §4), it stays ≥ ~5× worse. **Closed.** (This is worth recording because
it is the one place the microscale structure could have interacted with recursion
depth; it does not.)
...
- **I3 (additive split):** B̂ — mantissas AND scale plane — is wholly nonce-fresh;
  `C = Σ_b 2^{e_A+e_B}·(μ_A^b·μ_B^b)` has no nonce-invariant additive term. The
  (M, E) pair is multiplicative per-entry structure, not a cacheable split. Confirmed.
