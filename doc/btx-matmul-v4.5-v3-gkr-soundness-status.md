...
> **MatMul v4.7 use.** “V3” here is an internal historical proof/coupled
> construction label, not the network transition version. This work may inform
> Epochs B–D, but no proof is consensus-authoritative in Epoch A. Epoch B first
> requires a durable Profile-1 proof alongside ExactReplay; Epoch C is the
> separate Profile-1 authority cutover; Epoch D separately binds Profile 2.
...
1. Header/template/nonce<br>
2. Canonical packed-bank commitment + PCS page openings under `bank_root`<br>
3. Every selected page + full 1536 coverage (production V3)<br>
4. Every M=`rows_per_lobe` GEMM claim grounded to bank pages<br>
5. Accumulation / permutation / exchange / Extract AIR (verifier-defined)<br>
