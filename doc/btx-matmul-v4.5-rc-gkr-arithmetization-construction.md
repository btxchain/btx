...
> **MatMul v4.7 use.** This construction is input to the proof-security work
> for Epochs B–D; it is not an Epoch-A verifier. Until its complete Profile-1
> statement, transcript, durable encoding, and independent audit satisfy the
> roadmap, ExactReplay remains the only authority. A later Profile-2 proof
> requires its own statement binding and the separate Epoch-D height.
