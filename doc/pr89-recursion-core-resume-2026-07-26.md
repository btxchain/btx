...
(4) REAL-BLOCK end-to-end (harness: real matmul-4.6-v3 block+tx -> statement -> prove ->
...
4. REAL-BLOCK END-TO-END — [ENG + EXE]. Build the harness: real matmul-4.6-v3 block + tx
...
5. CONSENSUS + MINER INTEGRATION — [ENG]. Wire the GPU prover into the miner path and the
   verify into block-validation as the MANDATORY consensus authority (the PR-89 goal);
   serialization / wire-format / activation.
