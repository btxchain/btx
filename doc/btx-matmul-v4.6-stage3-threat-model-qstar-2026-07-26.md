...
> **MatMul v4.7 transition status:** historical security analysis for
> unfinished proof machinery. It informs possible Epochs B–D but grants no
> authority. Epoch A uses Profile-1 ExactReplay; a mandatory proof must be
> durable; Profile 2 is reserved for Epoch D. See
...
1. **A2 lane-independence — now PROVEN** (under ROM + Poseidon-binding; audit-input, not
   ...
   **m_f − q = 154 − 78 = 76 at q = 78** (fails 100 by 24; under the within-proof site union
   ≈ 51 < 64). Classification: NECESSARY, status proven-pending-verification — keep it on the
