...
| 3 | Persistent device-resident GPU | LT CUDA/HIP: qualified INT8 IMMA/MFMA runs direct s8 stages, exact four-radix `Y·H`, and Karatsuba-9 combine; scalar graphs/tiles on decline. MX Extract derives scales on-device; all devices default to one dense INT8 projection. SM100/120 and gfx950 expose exact four-pass logical `(mu,E8M0/32)` only behind `BTX_MATMUL_V4_LT_LOGICAL_MX=1` because it schedules 4x dense GEMMs. Chat still D2H (`device_hashing=false`) | Device SHA256d + B200/5090/MI350 **re-measure after Lever B** (do not claim ≥4×) |
| 4 | Native FP4/FP8 genuinely wired | **Not for LT.** Corrected BMX4C and LT scale layouts are contraction-aligned, but CUDA/HIP currently lower logical MX exactly to INT8 IMMA/MFMA. Generic portable grouped “MXFP4” is integer emulation; CUTLASS/tcgen05/device-FP8 remain fail-closed; planner labels are not execution proof | Real native kernel + sm_100a/MI355 M-t24 self-qualification |
