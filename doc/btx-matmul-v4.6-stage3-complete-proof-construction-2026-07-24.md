...
This document is the implementation contract for replacing the profile-2
sampled carrier as final authority. It does not claim that the complete proof
has already been built or reviewed.
...
The current code selects coupled **or** episode verification. Stage 3 requires:
...
process-local carrier state may never reject or accept a block. While Stage 3
is disabled, profile 2 falls through to ExactReplay. After an eventual Stage-3
cutover, ExactReplay remains diagnostic/reference code and is not a fallback
from a failed complete proof.
