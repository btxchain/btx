...
> **MatMul v4.7 transition status:** this is historical Profile-2 sampled-
> carrier measurement, not the Epoch-A verification budget or authority model.
> Epoch A uses complete Profile-1 ExactReplay. Epoch B requires a durable proof
> plus replay; Epoch C retains Profile 1 under proof authority; Profile 2 is
> reserved for Epoch D and is not an ExactReplay launch candidate. See
...
## 1. What the historical budget governs

The 900 ms figure bounds the historical Profile-2 sampled-carrier precheck. It
does not govern MatMul v4.7 Epoch-A Profile-1 ExactReplay, does not authorize a
block, and is not an activation gate for Profile 2. The canonical Epoch-A
performance gate measures full, byte-identical replay on qualified accelerator
paths.
...
**Current transition authority.** The sampled carrier is optional historical
relay/precheck state and never returns final consensus success. Epoch A selects
Profile 1 and complete ExactReplay. Profile 2 remains dormant until the
proof-authoritative Epoch-D height. Formal GKR/FRI and Stage-3 authorities
remain compile-time hard-disabled.
...
## 8. Historical result and current activation reading
...
This result closed the historical carrier-performance question only. It does
not clear the MatMul v4.7 Epoch-A ExactReplay gates and does not justify a
Profile-2 height. The current activation gates are listed in
`doc/btx-matmul-v4.7-transition-roadmap.md`; all heights stay disabled until
their separate activation reviews.
