...
> `doc/btx-matmul-v4.7-transition-roadmap.md`. Mainnet Epoch A (v4 = BMX4C = RC) remains a release candidate pending exact-final CUDA+Metal evidence, reviewed ASERT calibration, ratification, and selection of a live activation height; all other transition heights remain disabled.
...
## Historical proof-completion gate

This dated construction cannot participate in a future Epoch-B/C/D activation
unless all are true:
...
- proof data is canonically committed and durably available through relay,
  reorg, restart, and IBD rather than only an ephemeral sidecar;
- Epoch B first requires the proof **and** Profile-1 ExactReplay; and
- every public epoch height is changed only by its own explicit
  activation-height commit.
