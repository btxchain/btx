...
| Strassen / LCMA / adaptive limbs | Exact alternatives **allowed**; “≤12.5% / one level / absorbed” is a narrow-datapath artifact (audit F2) — calibrate ASERT to fastest known exact (`doc/btx-matmul-v4.4-combine-algorithm-tournament.md`); zero E2M1/E4M3-native levels on frontier FP pipes remains a datapath note, not a hardness claim | Same disposition; public adaptive-limb + Karatsuba-9 baselines published miner-local |
...
| Small-alphabet shortcuts | SAFE: tables/four-Russians/mailman ≥ 10²× losers (opening condition ≤ ~1.5 effective symbols); BNN cliff ~9×/18–27× below; 2:4 sparsity impossible by counting; zero-skip ≤ 9.1 % bespoke-only; Strassen/LCMA/adaptive-limb edges are **exact miner efficiency** (not a “≤12.5% closed cap” — audit F2); calibrate to fastest known exact | cryptanalysis §0/§7 verbatim (it analyzed 𝓜₁₁ as primary); §A.6 rewrite |
