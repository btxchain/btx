...
13. **Mod-q combine on tensor cores (OPEN — post-XOF-fix bottleneck).** With the wide XOF, the optimal miner's remaining non-tensor stage is the m×m×n combine `Ĉ = P·Q mod q` (P = U·A, Q = B·V, exact int32, |·| < 2³⁰): ~n³/64 128-bit-product MACs on **integer ALUs**, not tensor cores — measured 35.5% of per-nonce time on a 5090 at n=4096 pre-fix, and the *dominant* stage (~90%) post-fix. Planned fix: limb-decompose P and Q (e.g. five signed 7-bit limbs per int32, valid since n·(2⁶)² ≪ 2³¹), compute the ≤ 25 limb-pair m×n×m products as native s8×s8→s32 tensor GEMMs, then recombine with shifted mod-q folds so only the final m×m reduction runs on int ALU (O(m²) vs O(m²n)). Until this lands and is measured, the §K.2a wall-time dominance requirement is **not met by the combine stage on GPU backends** and difficulty/economics claims must use the measured stage split, not MAC counts.
