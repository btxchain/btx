...
| 8 | "If we can't efficiently do Freivalds it is a problem" | Deterministic Freivalds over the CRT-reconstructed modulus; 2–4 rounds for 2⁻¹²⁸; ms-scale verify at n = 4096 | §D |
...
The correct construction (§D.3), native to the exact-INT32 accumulation of the INT8 path: the committed product entries are **exact integers** (`|C_ij| ≤ n·127² < 2³⁰` for all in-range `n`), so run Freivalds over an **independent large prime `q = 2⁶¹ − 1`** (Mersenne; one 64-bit multiply-and-fold per MAC) — or over `GF((2³¹−1)²)` to reuse the existing `matmul::field` code. Because any two distinct canonical integer entries differ by `|Δ| < 2³² < q`, a wrong entry can never alias to a correct residue: **per-round error ≤ 1/q ≈ 2⁻⁶¹.**

**Normative round count: `R = 3`** → error ≤ 2⁻¹⁸³ (well past 2⁻¹²⁸; `R = 2` already gives 2⁻¹²²). This **supersedes the `R = 8` in §G.2**, which was computed under the incorrect small-field assumption. Update `nMatMulV4FreivaldsRounds` default to 3 (2 on regtest), and §G.4 invariant #5 to the `1/q` bound.
...
| `k` (CRT compute primes) | **4** → {251, 241, 239, 233} | INT8 tensor-core lanes; exact INT32 accumulation for n ≤ 34,359 (§B.4). |
...
The Freivalds witness `y = C·v mod M` (v a σ-derived vector) is the O(n²) handle validators use to confirm `C = A·B`; the full protocol is §D.
...
Solve(header, params, max_tries):
    ...
    while max_tries > 0:
        ...
        C   ← INT8_MATMUL(A, B)                            # Θ(k·n³) exact s8×s8→s32, no gate
...
Verify(header, params, height):                            # O(n²) — never recomputes A·B
    ...
    C ← block.matrix_c_data                                # shipped product, n² int32 words
    ...
    return Freivalds(A, B, C, sigma, R)                    # §D, O(R·k·n²)
```
...
### A.6 Per-nonce cost is a hard Θ(n³)

Per nonce the work is `k·n³` INT8 MACs plus `O(k·n²)` sampling/commit/witness — `k·n³·(1+o(1))`, with no path around the cubic term:
...
- **Strassen / sub-cubic.** Not forbidden, made non-remunerative: to pass Freivalds the miner must produce the *correct* dense `C`, and integer Strassen at n=4096 (2–3 feasible levels) saves ≤ ~1.2–1.3× while its block linear-combinations exceed the s8 input range (needing ≥ s16 operands no s8×s8→s32 MMA accepts) and its extra `Θ(n²)` add passes run on integer ALUs, not tensor cores. Any residual constant-factor edge is absorbed by difficulty calibration (§N-risk-ii), exactly like any miner optimization — not a break, because the unit of work is "the correct product," and `N³` tile products of `b³` MACs is the cheapest way to produce it on tensor cores.
...
### B.3 Candidate (ii): CRT residue decomposition over `k` small primes — **recommended**

```
p_1=251, p_2=241, p_3=239, p_4=233   →   M = 3,368,562,317 ≈ 2^31.65
```

Operands live in `Z_M`; by CRT `Z_M ≅ Z_251×Z_241×Z_239×Z_233`, sample each residue uniformly (byte rejection; 2.0/5.9/6.6/9.0%) and the element is exactly uniform over `Z_M` with no wide-integer arithmetic in the hot path. **Layout:** `k` planar u8 matrices per operand (each `n²` bytes) — residue-planar, so each channel is a standard u8 GEMM; the per-nonce matmul is `k` independent s32-accumulated INT8 GEMMs (the v3 "split16" 4×-GEMM idea, now on tensor cores), perfectly parallel across SMs/devices.
...
|---|---|---|---|
| u8, p=251 (recommended) | 250 | 62,500 | **34,359** |
| u8, raw [0,256) | 255 | 65,025 | 33,025 |
| p=127, [0,126] | 126 | 15,876 | 135,266 |
| p=127 balanced [−63,63] s8 | 63 | 3,969 | 541,064 |

So any `n ≤ 34,359` runs the whole reduction dimension in one un-reduced s32 accumulation (one reduction per output element). For `n > n_max`, split the K-dimension into panels of depth `≤ n_max`, reduce each to `[0,p_m)`, and sum panel residues before a final reduce; since `x ↦ x mod p_m` is a homomorphism, reduce-then-sum = sum-then-reduce, so **the panel partition is not consensus-visible**. The only consensus-fixed granularity is the commit tile `b`.

### B.5 Recombination: CRT reconstruction (Garner, u64)

Needed only where a `Z_M` value is serialized (the Freivalds witness `y = C·v`, `n` elements/nonce); never inside the GEMMs.
...
Inverse constants (each verified): 251⁻¹ mod 241 = 217; 251⁻¹ mod 239 = 20; 241⁻¹ mod 239 = 120; 251⁻¹ mod 233 = 13; 241⁻¹ mod 233 = 204; 239⁻¹ mod 233 = 39. Max reconstruction = M−1 = 3,368,562,316.
...
| Quantity | Value |
|---|---|
| Dimension, tile | n=4096, b=64, N=64 |
| Channels | k=4, {251,241,239,233}, M=3,368,562,317 |
| Operand storage | 8 residue matrices × n² B = **128 MiB**/nonce (u8, residue-planar) |
| MAC count | k·n³ = 4·2^36 ≈ 2.75×10¹¹ INT8 MACs (~5.5×10¹¹ ops)/nonce — sub-ms at INT8 tensor-core peak |
| Accumulator peak | 4096·250² = 2.56×10⁸ < 2^31−1; headroom ×8.4; no mid-K reduction |
| C payload | n² int32 = 64 MiB shipped for Freivalds/verification |
...
- **I5 — No pre-hash lottery.** `ε=0`; every nonce runs the full GEMM; the cheapest nonce costs Θ(k·n³), not one SHA-256d.
...
Together I1, I2, I3, I5, I6 make the marginal cost of a nonce equal the average cost — `k·n³` INT8 tensor-core MACs — with all identified v3 amortization channels structurally absent rather than parameter-disabled, and with verification held at O(n²).
...
The fix is native to the v4 compute path. INT8 inputs with exact INT32 accumulation mean the *true* product entries are exact integers: with balanced entries in [−127, 127], |C_ij| ≤ n·127² < 2³⁰ even at n = 32 768. Therefore:

1. **Commit the exact integer product** (or, in the k-prime CRT variant, have the verifier CRT-reconstruct the k committed planes back to the unique integer in [0, P), an O(k·n²) pass — valid because P > n·(p−1)² for all in-range n). Payload words are range-checked for canonicality, as v3 does at `src/pow.cpp:2867`.
2. **Run Freivalds over an independent large prime q = 2⁶¹−1** (Mersenne; one 64-bit multiply + fold per MAC), or over the quadratic extension GF((2³¹−1)²) if reusing the existing `matmul::field` code is preferred. Since any two distinct canonical committed values differ by |Δ| < 2³² < q, a wrong integer entry can never reduce to a matching residue: per-round error is exactly ≤ 1/q.
...
3. **Sketch-Freivalds rounds** — for t = 1..R = 3, derive (xₜ, yₜ) ∈ 𝔽_q^m from H(σ ‖ H(Ĉ)) and check
   ...
   **xₜᵀ · Ĉ · yₜ == (Uᵀxₜ)ᵀ · A · (B · (V·yₜ))**
   ...
   Right side: two dense O(n²) matvecs plus O(nm) projections; left side O(m²). If Ĉ ≠ U·C·V in even one word, each round catches it with probability ≥ 1 − 2/q (degree-2 Schwartz–Zippel over 𝔽_q): error ≤ 2⁻¹⁸⁰ at R = 3, matching §D.3.
...
Any linear commitment of C admits an algebraic evaluation shortcut: Ĉ = (U·A)·(B·V) can be computed in ≈ 2n²m + m²n MACs instead of n³ + 2n²m, a factor ≈ n/(2m) = b/2 (8× at b = 16). Stated plainly:

- The **de-facto per-nonce work unit** under this payload is a dense INT8 n×n×2m GEMM (≈ 8.6×10⁹ MACs at n = 4096, b = 16) — same tensor-core, same bandwidth profile as the full product, 1/8 the volume. Difficulty calibration and the memory-gate sizing in §M MUST assume this optimal algorithm, not the naïve n³ figure.
...
|---|---|---|
| Mining | ≈ 2n²m INT8-MACs per nonce (GPU tensor cores) + digest hash | working set per §M |
...
| `nMatMulV4PrimeCount` | uint32 | 4 | 4 | 4 | 4 | 4 | 4 | `k`; CRT primes. |
| `nMatMulV4TranscriptBlockSize` | uint32 | 64 | 64 | 64 | 64 | 64 | 64 | `b`; product-commit tile. |
| `nMatMulV4FreivaldsRounds` | uint32 | 8 | 8 | 8 | 8 | 8 | 2 | `R`; Freivalds rounds. |
| `nMatMulV4DigestScheme` | uint32 | 1 | 1 | 1 | 1 | 1 | 1 | Digest scheme (1 = SHA256d tile tree over exact-INT32 C, σ_v4 prefix). |
...
| `nMatMulV4MaxPendingVerifications` | uint32 | 4 | 4 | 4 | 4 | 8 | 16 | Pending cap (4 × 64 MiB = 256 MiB bound at n=4096). |
...
1. `n % b == 0` for every accepted dim in `[Min,Max]` (4096 % 64 = 0).
...
5. **Soundness:** worst-case per-round Freivalds false-accept `1/p_min = 1/233 ≈ 2^-7.86`; require `p_min^-R ≤ 2^-60` ⇒ `R ≥ 8` (233⁻⁸ ≈ 2⁻⁶²·⁹, matching v3's `< 2^-62`). (Higher soundness via more rounds or a larger effective modulus per §D.)
...
- 64 MiB payload makes full-block relay the dominant bandwidth cost; DoS budgets (I.5) and the pending cap are sized against it.
...
| `PopulateFreivaldsPayload` | `pow.cpp:2967` | v4 branch emits exact-INT32 C; never A/B. |
...
2. **Payload shape & canonicality** — `matrix_c_data.size() == n²`; A/B empty; every word `|C_ij| ≤ 15,625·n`.
...
4. **Deterministic Freivalds, R rounds** — for each round, `r` from `H(matmul_digest ‖ round ‖ lane)`; check `A·(B·r) ≡ C·r (mod p_i)` per lane. A,B **regenerated from seeds on the fly** (row-streamed): `B·r` then `A·(B·r)` is O(n²) time, O(n) memory beyond the payload. **No O(n³) recomputation anywhere in v4 verification.**
...
ASERT machinery **kept as-is** (`CalculateMatMulAsertTarget` `:1829`, `MatMulAsert` `:2106`, `GetNextWorkRequired` `:2455`; half-life 3,600 s, spacing 90 s). Target form unchanged (`nBits` bounds `matmul_digest`); acceptance probability per attempt is uniform in the target, so ASERT needs no formula change. What changes is the **work unit**: a v3 attempt was dominated by the 18-bit pre-hash gate with an occasional n=512 matmul; a v4 attempt always pays a full n=4096 matmul. Attempts/s drops by a large hardware-dependent factor at the fork, so apply a **one-time target rescale + ASERT re-anchor at `nMatMulV4Height`** (mechanically identical to `nMatMulAsertRetune2`, `params.h:247-252`): `next_target = parent_target × Num/Den`, then re-anchor. The ratio must be **calibrated empirically** pre-release (benchmark the v4 reference miner vs observed v3 throughput) and encoded per network at tag time. Fresh networks set genesis/bootstrap `nBits` for the v4 work unit and leave rescale at 1/1. `btx-genesis.cpp`/`CreateBTXGenesisBlock` are untouched — genesis remains a pre-fork object.

### I.5 DoS budget retuning

Baseline: v3 Freivalds n=512 R=2 = 569 µs ⇒ 284.5 µs/round at n=512; per-round scales `(n/512)²`.

| Component (n=4096, R=8, k=4) | Cost |
|---|---|
| Freivalds/round | 284.5 µs × 64 ≈ **18.2 ms** |
| Freivalds total (R=8) | ≈ **146 ms** |
| Digest over 64 MiB (SHA256d) | ≈ 45–130 ms |
| Seed expansion of A,B (2n² ≈ 33.5 M) | ≈ 30–60 ms |
| **Total/verification** | ≈ **220–340 ms; budget at 300 ms** |
...
- `nMatMulV4MaxPendingVerifications = 4` (was 16) — pending payload ≤ 256 MiB.
- Failure handling unchanged: misbehavior scores 20/100 (`pow.h:134-135`), ban threshold 1. Motivation for threshold 1: a bogus 64 MiB payload costs the attacker 64 MiB upload but the victim only ~0.3 s CPU — bandwidth, not CPU, is binding, bounded by the budget + pending caps.
...
| 12 | `src/matmul/freivalds.{h,cpp}` | — | Multi-lane deterministic Freivalds over `{p_i}` with streamed seed-regenerated A/B (O(n) memory); round/lane challenge derivation. |
| 13 | `src/matmul/transcript.{h,cpp}` | — | v4 digest: σ_v4 + b×b tile hash of exact-INT32 C (scheme 1). Legacy `CanonicalMatMul` retained for pre-fork validation. |
...
**Editor's load-bearing findings:** (1) v3 payload validators cap dim at 2048 (`pow.cpp:137`, `params.h:143`), *below* v4's 4096 — height-selected bounds are mandatory. (2) The 64 MiB C payload at n=4096 exceeds `nMaxBlockSerializedSize` (24 MB) and `MAX_PROTOCOL_MESSAGE_LENGTH` (16 MB), and n=8192 exceeds the `MAX_SIZE` element cap — H.3/J#7-8 are explicit fork deliverables.
...
(Ratios are n-independent while compute-bound; §L. Even crediting CMP 170HX its 42 TFLOPS FP16 — unusable, breaks determinism — it trails H100 ~47×.)
...
**Recommendation: n = 4096, k = 4, b = 64, single-C Freivalds verification.** n=8192 is a governance-raisable option after the §H.3 plumbing lands; larger n and the capacity gate are deferred pending a verification-preserving construction.

### M.2 k — CRT prime count

Each prime `p_j < 2⁸` adds one s8×s8→s32 GEMM (+2n³ ops) and per-prime residue operands/accumulators; throughput rescales uniformly by 1/k without disturbing the §K.3 ratios. Soundness (per-round error ~1/p per prime, driving R) fixes minimum k; k = 4 gives effective modulus M ≈ 2³¹·⁶⁵ ≥ v3's field. Exact primes/CRT/(k,R) soundness owned by the arithmetic/verification sections (§B, §D).

### M.3 b — commit tile size

C is committed in b×b tiles. Constraints: (i) hashing ≪ 1% of tensor time (large b + algebraic tile compression, cf. v3 §8.3.1) so the digest never becomes a v3-style SHA side-channel; (ii) verification openings transmit O(b²) words, favoring small b. The commitment must be computable from the natural tile-streaming schedule (§L.2) without forcing extra HBM traffic. b = 64 at n=4096. Constraint: hash+commit < 1% of 2kn³ ops; opening payload < ~100 KB/challenge.

### M.4 Block time and work-unit sizing

Retain **90 s**. Work unit = one nonce = 2kn³ ops; the difficulty target on the product-committed digest now tracks **aggregate network dense-INT8 TOPS**. `R ≈ ε·P_int8/(2kn³)`: at n=4096, k=4, ε=0.65, H100 ≈ 1,100 nonces/s → ~10⁵/block (excellent variance). Difficulty cadence/clamps carry over from v3, with the one-time fork rescale of §I.4. As successors raise P_int8, difficulty rises transparently; governance retargets n only when VRAM boundaries shift, not to chase FLOPS.
...
| iii | **Field too small for soundness** (per-round Freivalds ~1/p ≈ 2⁻⁸/prime). | Low | High if mis-specified | Run Freivalds over the CRT-reconstructed modulus M with R rounds (§D): R=8 → ≤ 2⁻⁶²; R=16 → ≤ 2⁻¹²⁸. Consensus tests encode the bound. |
...
| vi | **Verification/DA cost at large n** (Freivalds O(n²) but verifier regenerates 2n² operand bytes + downloads C). | Medium | Medium–High | Hard invariant (§0.7): full-node verify < ~0.3 s CPU, payload ≤ a few MiB→64 MiB at n=4096; caps n growth (raise k before n; n≤8192 only after §H.3); stream PRF regeneration (O(n) RAM on verifiers). |
...
| Term | Definition |
|---|---|
| **n** | Matrix dimension. v4 default 4096 (v3: 512). Per-nonce work is Θ(n³). |
...
| **P** | CRT-reconstructed modulus ≈ ∏ pᵢ; the field over which Freivalds soundness is evaluated. |
| **b** | Transcript / product-commitment block (tile) size; n must be divisible by b. |
...
| **Freivalds check** | O(n²) deterministic test C·r ≟ A·(B·r) for a header-derived vector r. |
...
2. **Final prime set {pᵢ}** and k — fixed by the arithmetic design (§B) after cross-vendor determinism testing.
