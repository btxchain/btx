## Status: historical design — MatMul v4.7 roadmap takes precedence

This document preserves an earlier v4 design stage and its version-local
findings. It is not the current activation plan. The proposed transition is
defined by `doc/btx-matmul-v4.7-transition-roadmap.md`: Epoch A uses
...
under proof authority. Mainnet Epoch A (v4 = BMX4C = RC) has a finite compiled candidate height and true source flags, so it would activate if merged unchanged; release remains NO-GO pending exact-final combined-tree CUDA+Metal evidence, reviewed schema-4 ASERT calibration, ratification re-affirmation, full closeout, and live-tip runway validation; all other transition heights remain disabled. Statements
...
|---|---|---|---|
| 1 | Consumer 5090 is the most efficient miner, at parity with H100/H200 | Invert via INT8 tensor-core dense matmul at large n (v4.1: b=4 + cross-nonce batched dense GEMM, §K.2b — the earlier b=8 profile was MEASURED consumer-favoring and the datacenter claim is now measurement-gated); datacenter wins per card (H100 ≈ 2.4×, B200 ≈ 5.4× a 5090) and per joule. No capacity gate (closed, §L.4) — the lever is compute + energy | §K, §L, §M |
...
| 10 | Must scale compute; worst case is compute going down via memory-hardness | Compute-bound above the roofline ridge (AI_opt = 2n/b = 2,048 at v4.1's b=4, §K.2a/§K.2b); never bandwidth-bound; no capacity gate (§L.4). More tensor FLOPS always → more reward | §L |
...
|---|---|---|---|---|
| **Sketch (default)** — ship `Ĉ = U·C·V ∈ 𝔽_q^{m×m}`, `m=n/b`, balanced-s8 template-derived `U,V` (§A.2 v4.1/I1′); digest `H(σ‖Ĉ)`; sketch-Freivalds `xᵀĈy ≟ (Uᵀx)ᵀA(B(Vy))` (§E) | **8 MiB** (b=4, m=1024) | **~0.1 s** | dense INT8 marginal unit `B·V` + stacked combine ≈ `1.25n³` MACs per nonce (§E.3/§K.2b) | **fits existing 16 MB/24 MB limits — no §H.3 protocol-limit changes needed** |
...
**Honest work-binding disclosure (§E.3, updated for v4.1/I1′):** under the sketch, the optimal miner computes `Ĉ = (U·A)(B·V)` directly — it need not form all of `C` — and under the v4.1 batched-sketch profile it additionally amortizes `U·A` and the A/U/V expansions across the whole nonce sweep of a template (§K.2b). This does **not** weaken verification soundness (no invalid block passes; §E.3), but it means the per-nonce *work unit* is the **marginal** cost — expand B + `B·V` + combine + digest — not `n³` and not even `2n²m`; **difficulty calibration (§I.4) and the §M work-unit/economics MUST use the measured marginal figure (matmul_v4_stage_bench).** Strict `n³` binding is available by choosing the full-C profile or the optional ZK module (§F.4d); it is not required to meet any stated objective. The I1′ anti-amortization relaxation that enables this is flagged NEEDS-EXTERNAL-REVIEW in §C.
...
| `b` (commit/sketch tile) | **4** (m = n/b = 1024 at n=4096); if n is retargeted to 8192, retarget b to 8 (m stays 1024, payload stays 8 MiB) | **Revised 16 → 8 → 4.** 16→8 was the §K.2a roofline fix (AI_opt = 2n/b had to clear every ridge). 8→4 is the **v4.1 batched-sketch profile** (§K.2b, PR #89): at b=8 the reviewer *measured* the workload consumer-favoring (H100/5090 = 0.40× nonce-throughput, n=8192) — the per-nonce GEMMs are too skinny and the SHA operand floor too large. b=4 doubles m, raising the marginal per-nonce tensor volume to ≈1.25n³ MACs (B·V + C-13 limb combine; U·A template-amortized under I1′), and the §K.2b batching fuses the combines into one large dense GEMM. **Whether this restores the datacenter ordering is unmeasured — real-GPU confirmation required (§K.2b GO/NO-GO)**. Sets sketch dimension m and payload 8m² bytes (§E.1). |
...
| Payload (default) | **8 MiB** sketch at n=4096, b=4 (`8m²`, m=1024) | Within existing 16/24 MB limits; no protocol-message-size fork needed. (Was 2 MiB at b=8, 512 KiB at b=16.) |
| Per-nonce work unit (difficulty/§I.4/§M.4) | **MARGINAL per-nonce work under I1′ (§K.2b): expand B (≈n²/31 SHA compressions) + `B·V` (`n²m = n³/4`) + combine (`n·m²` ALU-direct or `16·n·m² = n³` limb-tensor MACs) + digest** — at n=4096, b=4 ≈ 1.7×10¹⁰ tensor MACs + n³ combine MACs if the limb path is taken | Optimal-miner **marginal** basis (§E.3/§K.2b): `U·A` and the A/U/V expansions are template-amortized and MUST NOT be priced into difficulty. The floor and hardware ordering are **invariant** to this figure (work-unit-neutrality theorem, §L.2.1 / §S.4.3) — it sets difficulty calibration, not economics. |
...
**v4.1 batched-sketch revision (normative; supersedes the v4.0 both-operands-nonce-fresh derivation below — see §K.2b for the motivation and §C I1′ for the security argument and its needs-review status).** Define the **template hash**

```
template_hash = MatMulHeaderHash( header with nNonce64 := 0, nNonce := 0,
                                  seed_a := 0, seed_b := 0 )
```

(the `seed_a`/`seed_b` header fields are themselves nonce-derived under §H.4, so they MUST be zeroed too or the "template" projection would still vary per nonce). What the template hash binds: `nVersion, hashPrevBlock, hashMerkleRoot, nTime, nBits, matmul_dim` — it exists only once the parent block and the concrete template exist. Then:

```
seed_A          = SHA256( "BTX_MATMUL_SEED_V4"    || template_hash    || 0x41 )   # TEMPLATE-scoped
seed_B          = SHA256( "BTX_MATMUL_SEED_V4"    || full_header_hash || 0x42 )   # NONCE-fresh
seed_U          = SHA256( "BTX_MATMUL_V4_SKETCH_U" || template_hash )             # TEMPLATE-scoped
seed_V          = SHA256( "BTX_MATMUL_V4_SKETCH_V" || template_hash )             # TEMPLATE-scoped
sigma           = SHA256d(header)                                                 # NONCE-fresh (v3 rule)
```

`A`, `U`, `V` are constant across a miner's nonce sweep of one template; `B` and `σ` (and hence the digest `H(σ‖Ĉ)` and every Fiat–Shamir Freivalds challenge, which binds `σ` and the payload hash) remain nonce-fresh. This is what lets the miner compute `P = U·A` once per template and batch Q nonces' combines into the single dense GEMM `P·[B₁·V | … | B_Q·V]` (§K.2b, `src/matmul/matmul_v4_batch.{h,cpp}`).

The original v4.0 derivation (both operands nonce-fresh) for reference:
...
Under v4.1, only `seed_B`'s binding keeps the "every field including `nNonce64`" form; **every nonce attempt still defines an independent right operand `B` and an independent σ**, which is the per-nonce marginal work difficulty prices (I1′). Operand entries are expanded from `seed_A`/`seed_B` by a deterministic **wide counter-mode XOF** with per-byte rejection sampling into the canonical range of the arithmetic (section B.2/B.3): keystream block `j = SHA256(seed_le ‖ domain ‖ LE64(j))`, all 32 output bytes consumed in stream order (~31.4 accepted elements per compression at p = 251). **Normative requirement: the XOF MUST be wide (many elements per hash invocation). One hash per element is forbidden** — it prices the per-nonce work in SHA-256 compressions (`≈ (2n²+2mn)·256/251 ≈ 38.5M` at n = 4096, vs ~1.2M wide) and hands the workload back to hash-strong/int-ALU hardware, recreating the v3 failure mode v4 exists to kill (this was the PR #89 implementation bug found by review).

**Cost accounting (corrected — price time, not bytes).** Expansion is `O(k·n²)` *bytes*, but bytes are not the unit that competes with the GEMM: one SHA-256 compression and one INT8 tensor MAC differ by roughly four orders of magnitude in per-cycle hardware throughput, so `n²`-order hashes can dominate `n²m`-order MACs in wall-time. The binding requirement is §K.2a/§M.3: **the INT8 GEMM must dominate per-nonce wall-time on reference hardware, measured, not inferred from operation counts.** With the wide XOF, expansion is ~1.2M compressions vs ~1.7×10¹⁰ MACs at n = 4096 — subdominant on tensor hardware but still a real miner-side cost (all current GPU backends expand operands on the host CPU; see `src/cuda/matmul_v4_accel.cu`, `src/metal/matmul_v4_accel.mm`).
...
| Quantity | Value |
|---|---|
| Dimension, tile | n=4096, **b=4, m=1024** (v4.1; baseline k=1; N is per-lane) |
...
| Payload | sketch Ĉ: 8·m² = **8 MiB** (default, b=4, §E.1); full-C alternative: n² int32 = 64 MiB |
...
v4's posture: **every accepted digest commits the canonical integer transcript result.** Miners may use any *exact* algorithm (classical GEMM, Strassen, adaptive limbs, exact FP64/FP8/MXFP4 encodings, ASIC, …) that reproduces those bytes. There is **no practical lower-bound proof** that the reference algorithm is the cheapest exact path; calibration and ASERT should track the fastest known exact implementation. Approximate floating-point matching is forbidden.

- **I1 (v4.0, superseded by I1′ below in the v4.1 batched-sketch profile) — Nonce-fresh operands (with per-block memorylessness).** `seed_A/seed_B` commit to `(hashPrevBlock, height, nVersion, hashMerkleRoot, nTime, nBits, nNonce64, matmul_dim, parent_mtp, which)` (A.2). No operand/product/partial shared across nonces; extends the v3 nonce-fold fix. **Corollary (rent-and-dump bound):** because the seed binds `hashPrevBlock` (and parent MTP), *nothing* is computable before the parent block exists — a rent-burst attacker gets zero head start, cannot pre-mine during cheap-rental idle windows, and cannot stockpile work across blocks. This per-block memorylessness is the strongest non-batchability property available to a progress-free PoW, and combined with ASERT (§I.4, 3,600 s half-life) it is the complete in-model bound on spot-rental mine-and-dump (§S.4.4).
- **I1′ — v4.1 batched-sketch relaxation (NORMATIVE at/above the v4.1 profile; ⚠ NEEDS EXTERNAL ADVERSARIAL REVIEW — this deliberately relaxes I1/I7 anti-amortization and is stated as needs-review, NOT proven-safe).** `B` is nonce-fresh (binds every header field incl. `nNonce64`); `A`, `U`, `V` are TEMPLATE-scoped (bind the §A.2 template hash — every header field EXCEPT the nonce and the nonce-derived §H.4 seed fields); the per-nonce **marginal** work — expand `B`, `B·V`, the `P·Q` combine, the digest — is Freivalds-bound and is what difficulty MUST price (§I.4, measured via `matmul_v4_stage_bench`, never inferred from MAC counts). Security argument, part by part:
  1. **Soundness preserved.** The verifier is unchanged: the ONE winning nonce's `Ĉ` is independently Freivalds-verified in O(n²) with challenges Fiat–Shamir-derived from `H(σ‖H(Ĉ))` — σ and the challenges remain nonce-fresh, so per-round error stays ≤ 2/q and no invalid block passes. Batching is a miner-only optimization; the batched digest is byte-identical to the single-nonce digest for the same (header, nonce) (`matmul_v4_batch_tests`).
  2. **No pre-mining.** `A`/`U`/`V` bind `hashPrevBlock`/`hashMerkleRoot`/`nTime`/`nBits` through the template hash, so nothing template-scoped is computable before the parent block and the concrete template exist. The I1 memorylessness corollary survives at TEMPLATE granularity (weaker than v4.0's per-NONCE granularity — that weakening is precisely the relaxation).
  3. **Symmetric across miners.** Every miner of a given template derives the same `A`, `U`, `V` and pays the same one-time `P = U·A`; amortization is available to everyone equally (including pool participants — the template-scoped objects are derivable from the pooled work unit), so it confers no asymmetric advantage, only a uniform rescale of per-nonce cost.
  4. **Difficulty prices marginal work.** Because `U·A` is amortized, the per-nonce work unit for difficulty calibration (§I.4/§M.4) is the MARGINAL unit (expand B + `B·V` + combine + digest), not `2n²m`. Calibrating against the v4.0 unit would overstate per-nonce cost by the amortized share and mis-set the ASERT rescale (ACTIVATION B2b).
  - **What is NOT claimed:** we do not claim the marginal unit has no further shortcut. Known floor: any correct `Ĉᵢ` computation must read all of `Bᵢ` (n² fresh pseudorandom bytes ⇒ the XOF expansion is unavoidable) and `M·Bᵢ·V` with fixed rank-m `M = U·A`, `V` has no known sub-`n²m` evaluation for dense pseudorandom `Bᵢ` — but "no known" is not a proof, and template-scoped `U`,`V` reopen exactly the projector-cache channel v4.0's I7 existed to close. **External review of this relaxation is a mainnet-activation blocker (ACTIVATION B4).**
...
- **I7 (v4.0, PARTIALLY superseded by I1′) — Nonce-fresh sketch challenge.** v4.0 required the sketch projectors `U, V` AND the Freivalds vectors to be nonce-fresh, foreclosing every cross-nonce projection cache (a template-constant `U` makes `U·A`-style objects reusable across nonces). **v4.1 deliberately inverts the projector half:** `U`, `V` are template-scoped so `U·A` IS reusable across a template's nonce sweep — that is the §K.2b batching lever — and the cost of a nonce is re-based on the marginal unit (I1′-4). What survives of I7, unweakened and still normative: the Freivalds challenge vectors `x, y` MUST derive from `H(σ ‖ H(Ĉ))` with σ = SHA256d(full header, nNonce64 included) — nonce-fresh and payload-bound (Fiat–Shamir, §D.1/§E.2) — so verification soundness is untouched.
...
| Sketch evaluation shortcut `(U·A)(B·V)` (v4-specific) | avoids forming C; v4.1 additionally amortizes `U·A` per template (I1′) | acknowledged, not closed — DESIGNED-IN under v4.1: same dense INT8 tensor-GEMM profile, difficulty prices the marginal unit (§0.7-(3), §E.3, §I.4, §K.2b); strict n³ binding needs full-C or ZK (§F.4d) |
...
| Cross-nonce sketch-projector cache | reuse `U·A`-type objects if `U` is template-constant | **v4.1: intentionally OPEN (I1′)** — `U·A` amortization is now the design (the §K.2b dense-GEMM batching); difficulty prices the marginal unit instead. Freivalds challenges remain nonce-fresh (I7 residue). ⚠ needs external review |

Together I1′/I2–I3 and I5–I8 pin the MARGINAL cost of a nonce (v4.1: template-scoped work amortizes, so marginal < average by design — the inverse of v4.0's marginal-equals-average posture; difficulty is calibrated to the marginal unit) — a dense INT8 tensor-core GEMM of `k·n³·(2/b)` MACs under the default sketch payload (`k·n³` under full-C; §E.3) — with all identified v3 amortization channels (and the resource-footprint and projector-cache channels a would-be capacity gate would open) structurally absent rather than parameter-disabled, and with verification held at O(n²).
...
| **Sketch Ĉ, b = 4 (m = n/4) — v4.1 normative at n=4096** | 8m² | **8 MiB** | 32 MiB | 128 MiB | 512 MiB |
| Sketch Ĉ, b = 8 (m = n/8) — normative at n=8192 (m stays 1024) | 8m² | 2 MiB | **8 MiB** | 32 MiB | 128 MiB |
| Sketch Ĉ, b = 16 | 8m² | 512 KiB | 2 MiB | 8 MiB | 32 MiB |
| Sketch Ĉ, b = 32 | 8m² | 128 KiB | 512 KiB | 2 MiB | 8 MiB |

**v4 ships only the compressed per-block commitment**: the sketch **Ĉ = U·C·V ∈ 𝔽_q^{m×m}**, m = n/b, where U (m×n) and V (n×m) are dense TEMPLATE-derived pseudorandom matrices (§A.2 v4.1/I1′; v4.0 derived them from σ — the v4.1 change is what enables the §K.2b batching), words canonical mod q = 2⁶¹−1 (8 bytes). The header digest is `matmul_digest = H(σ ‖ Ĉ)`, recomputed by every verifier in O(m²). Recommended and normative network-wide (v4.1): **b = 4 at n = 4096 (8 MiB payload, m = 1024); b = 8 if n is retargeted to 8192 (m stays 1024, payload stays 8 MiB)** (`nMatMulV4TranscriptBlockSize = 4`, §G.2) — within the existing 16 MiB message budget. The b choice history: b = 16 → 8 was roofline-driven (§K.2a: AI_opt = 2n/b must clear every device ridge). b = 8 → 4 is measurement-driven (§K.2b, PR #89): at b = 8 the reviewer measured H100/5090 = 0.40× — the per-nonce tensor volume was too small relative to the SHA/int-ALU floor, and the ordering collapsed regardless of AI. b = 4 doubles m, quadrupling the combine volume and doubling `B·V`, and the §K.2b batched evaluation makes the enforced shape one LARGE dense GEMM. ⚠ The AI-model-based claims for both earlier b choices were falsified by measurement; the b = 4 datacenter claim is a HYPOTHESIS pending the §K.2b real-GPU GO/NO-GO.
...
1. **Regenerate A, B from the header seeds** — O(n²) PRF expansion (v3's `SharedFromSeed` path, cf. `src/pow.cpp:2854-2855`); ~15–35 ms at n = 4096. *(Envelope caveat: 15–35 ms was estimated for a ChaCha-class XOF. The pinned wide SHA-256 counter-mode XOF (§A.2/C-12) needs ~1.07M single-block compressions for A+B at n = 4096 — order 50–150 ms single-thread with SHA-NI, several hundred ms without; measured ~460 ms on a non-SHA-NI reference core. Still comfortably inside the §D.5/§I.5 sub-second verify budget, but the 15–35 ms figure must be re-benchmarked on SHA-NI reference CPUs, ACTIVATION B2d.)* The verifier *never* needs the O(n³) product: A·(B·r) reproduces any projection of C directly.
...
Any linear commitment of C admits an algebraic evaluation shortcut: Ĉ = (U·A)·(B·V) can be computed in ≈ 2n²m + nm² MACs (U·A and B·V at n²m each, their m×n by n×m product at nm²) instead of the honest n³ + n²m + nm². **v4.1 (I1′/§K.2b) goes one deliberate step further:** with A, U, V template-scoped, `U·A` (and the A/U/V expansions) amortize across the whole nonce sweep, so the per-nonce **marginal** unit is

```
expand B (SHA XOF, ≈ n²·(256/251)/32 compressions)
+ B·V        (n²m MACs, tensor)
+ combine P·Q (n·m² exact-int32 mod-q MACs ALU-direct, or 16·n·m² s8 MACs on the C-13 limb-tensor path)
+ serialize + digest (SHA over 8m² bytes)
```

At n = 4096, b = 4 (m = 1024): `B·V` = n³/4 ≈ 1.72×10¹⁰ MACs; the limb-tensor combine adds ≈ n³ ≈ 6.9×10¹⁰ s8 MACs on tensor units (or n³/16 on int ALUs, whichever measures faster per device). Stated plainly:

- The **de-facto per-nonce work unit** under this payload is the MARGINAL unit above — and in the §K.2b batched evaluation its combine runs as one large dense GEMM `P·[B₁·V | … | B_Q·V]` (m × Q·m × n). Difficulty calibration in §M/§I.4 MUST assume this optimal batched-marginal algorithm — measured with `matmul_v4_stage_bench` per §K.2a-WT, never inferred from MAC counts — not the naïve n³ figure and not the unamortized 2n²m figure.
...
| `nMatMulV4TranscriptBlockSize` | uint32 | 4 | 4 | 4 | 4 | 4 | 4 | `b`; product-commit/sketch tile, m = n/b (§0.7, §E.1/§K.2b). **4** (was 8, was 16): v4.1 batched-sketch profile — raises the marginal per-nonce tensor volume after the PR #89 measurement falsified the b=8 ordering; if n is retargeted to 8192, set b=8 so m stays 1024. |
...
| `nMatMulV4MaxPendingVerifications` | uint32 | 4 | 4 | 4 | 4 | 8 | 16 | Pending cap (4 × 8 MiB = 32 MiB bound at n=4096 sketch, b=4; 256 MiB under full-C). |
...
> **Profile note (§0.7-(3)):** H.2–H.3 specify the **full-C (strict-binding) profile**. Under the **default sketch profile** the payload is `Ĉ ∈ 𝔽_q^{m×m}` (8·m² bytes = 8 MiB at n = 4096, b = 4; §E.1), which fits every existing size limit — none of the H.3 size-limit changes are then required.
...
### K.2a Roofline on the optimal-miner basis (historical rationale for b=8 — superseded by §K.2b's measured b=4 batched profile; the AI-clearance requirement still applies, and b=4 halves AI_opt to 2n/b = 2,048 at n=4096, still above every ridge)
...
**K.2a-WT (normative, added after the PR #89 review): the roofline argument above only orders devices by INT8 TOPS if the tensor GEMM dominates per-nonce WALL-TIME. MAC counts and byte counts do not establish this.** The per-nonce pipeline has three stages on different hardware units — operand expansion (SHA/int units), the two INT8 GEMMs (tensor cores), and the mod-q combine (int ALU) — and any stage priced on non-tensor units erodes the datacenter ordering in proportion to its wall-time share. Requirement: on every reference device, the measured tensor-GEMM share of per-nonce wall-time MUST be the strict majority, and the SHA + int-ALU shares MUST be re-measured (per-stage timers, bit-exact against the reference digest) whenever the XOF, tile b, or combine implementation changes. Status at the time of writing: the wide XOF (§A.2) brings expansion to ~2–5%; the mod-q combine does NOT yet meet this requirement on GPU backends (measured ~35.5% pre-XOF-fix, dominant post-fix at n=4096 on a 5090) — the limb-decomposition mapping of Appendix C-13 is the designated fix.
...
At **b=16**, `AI_opt = 512` sits *below* the H100 (591), B200 (563), and 4090 (655) ridges — attainable throughput is bandwidth-clipped to `512×BW`: H100 87 %, B200 91 %, 4090 78 % — while consumer/A100 cards (lower ridges) run at 100 %. Net, the H100:5090 ordering slope erodes from 2.36× toward ~2.05×: **b=16 silently hands ~9–22 % of the datacenter lever to bandwidth-rich consumer cards**, hidden by quoting AI on the wrong (n/3) basis. At **b=8**, `AI_opt = 1,024 ≥ 1.56×` above *every* ridge, restoring the full compute ordering. This was the rationale for the §0.7 `b = 16 → 8` change. **Superseded caution (§K.2b):** real-hardware measurement at b=8 nevertheless showed a consumer-favoring ordering (H100/5090 = 0.40×) — AI clearance is necessary, not sufficient; per-launch GEMM volume and the non-tensor floor decide the ordering. The v4.1 profile is b = 4 (AI_opt = 2,048, still above every ridge) with cross-nonce batching; all such roofline arguments are now advisory only, subordinate to the §K.2b GO/NO-GO measurements.

### K.2b The batched-sketch profile (v4.1) — why b=4 + template-scoped A/U/V, and what is still unproven

**Measured problem (real hardware, PR #89 review — the anchor for everything in this section).** At b = 8, n = 8192, the reviewer measured **RTX 5090 ≈ 161 nonce/s vs H100 ≈ 64 nonce/s (H100/5090 = 0.40×)** — the v4 sketch workload as then specified was *consumer-favoring*, inverting the design objective. Root cause, from the reviewer's per-stage timers: the per-nonce GEMMs (`U·A`, `B·V` at m = n/8, plus an m×m×n combine) are **skinny** — too little tensor volume per launch to reach a datacenter part's dense-GEMM operating point — while the per-nonce SHA-XOF operand generation and int-ALU mod-q combine sit on exactly the units where high-clock consumer silicon is strongest. Two prior parameter choices (b = 16 → 8) were justified by AI/roofline models that this measurement falsified. **Consequence adopted here: no model output in this section is load-bearing; only the GO/NO-GO measurements below are.**

**The v4.1 response — restructure the enforced shape, not just the parameters:**

1. **Template-scoped A, U, V (§A.2 v4.1, invariant I1′).** `seed_A`, `seed_U`, `seed_V` bind the template hash (every header field except the nonce and the nonce-derived §H.4 seed fields); `seed_B` and σ stay nonce-fresh. A miner therefore expands A/U/V and computes `P = U·A` **once per template**.
2. **Cross-nonce batching into one large dense GEMM.** Per window of Q nonces the miner computes the nonce-fresh right factors `Qᵢ = Bᵢ·V` (stackable as `[B₁; …; B_Q]·V`, a Q·n × n × m GEMM) and then evaluates ALL Q combines as the single dense GEMM `P·[Q₁ | … | Q_Q]` (m × Q·m × n; via the C-13 limb path, 16 such limb-pair GEMMs). At n = 4096, b = 4, Q = 32 that is a 1024 × 32768 × 4096 s8 GEMM per limb pair — the dense, large, batched shape datacenter INT8 tensor pipelines are built for, replacing Q skinny per-nonce launches.
3. **b = 8 → 4** doubles m: quadruples the combine volume and doubles `B·V`, raising the marginal per-nonce tensor volume to ≈ 1.25n³ MACs so the SHA/int-ALU floor (unchanged in absolute terms: one B expansion + digest per nonce) shrinks as a *share* of per-nonce time. Payload: 8m² = 8 MiB at n = 4096 (§E.1), inside existing message limits.
4. **Per-nonce SHA floor halves:** only B is expanded per nonce (A was half the per-nonce XOF cost in v4.0).

**Wall-time note (extends §K.2a-WT to the batched profile — normative).** The §K.2a-WT majority requirement is re-based on the **marginal** unit: on every reference device, the measured tensor share (S2 `B·V` + S3b limb-pair GEMMs) of per-nonce **marginal** wall-time MUST be the strict majority at window Q ≥ 32, with the template stage S0 excluded (amortized) and every stage output bit-exact against the reference digest. The normative stage boundaries (S0, S1b, S2, S3a/S3b/S3c, S3′, S4) are pinned in `src/bench/matmul_v4_stage_bench.cpp`, which is the required measurement harness; GPU backends must instrument the same boundaries on the stacked window shapes.

**GO/NO-GO (must be measured on real silicon before any datacenter-ordering claim, and before mainnet activation — ACTIVATION B2g):**

- (a) H100 and B200: (S2 + S3b) ≥ strict majority of marginal per-nonce wall-time at Q ≥ 32, n = 4096, b = 4;
- (b) H100 and B200: implied INT8 tensor utilization of the batched S2+S3b GEMMs ≥ ~60% of device peak;
- (c) the resulting nonce/s ordering actually places H100 and B200 above an RTX 5090 by a margin that survives price normalization (§S.4.6 ρ re-measure);
- (d) verify-side unchanged: O(n²) sketch-Freivalds at b = 4 (m = 1024) still inside the §D.5 budget on reference CPUs (the Freivalds matvecs are b-independent; the m² payload SHA grows 4× to ~8 MiB — re-bench).

**Status: DESIGN CANDIDATE.** The batched-sketch profile is implemented end-to-end on the CPU reference (`matmul_v4_batch.{h,cpp}`, byte-identical to the single-nonce consensus path, verifier untouched) and the shape argument above says *why it targets the measured root cause* — but the two previous rounds of AI-model estimates for this workload were both wrong, and **no real-GPU measurement of the v4.1 profile exists yet**. Until (a)–(d) are measured on physical H100/B200 (and ideally the reviewer's 5090), the datacenter-favoring claim is a hypothesis, not a result. The I1′ anti-amortization relaxation this profile rests on additionally requires external adversarial review (§C I1′, ACTIVATION B4).
...
*(Times shown are for the full n³ product; the v4.1 optimal miner pays the MARGINAL unit of §E.3/§K.2b. Payload figures in the last column are full-C; the default sketch payload is 8·(n/b)² B — 8 MiB at n = 4096, b = 4, §E.1.)*
...
C is committed via the dense sketch Ĉ = U·C·V, m = n/b (§E.1). b trades three quantities: (i) payload 8·m² = 8·(n/b)² bytes shrinks with b; (ii) the §E.3 work-shortcut factor b/2 grows with b; (iii) commitment granularity m² coarsens with b. **b = 4 network-wide at n = 4096** (§G.2/§K.2b batched-sketch profile, m = 1024, payload 8 MiB; b = 8 if n is retargeted to 8192 so m stays 1024); see §E.1 for the rejected corners.

**Wall-time invariant (revised per the PR #89 review — measure, don't count).** Every non-tensor per-nonce cost (seed/seal SHA, operand-expansion XOF, and the mod-q combine) must stay ≪ 1% — and in aggregate strictly subdominant to the tensor GEMM — **as measured per-stage wall-time on reference hardware (§K.2a-WT), never argued from operation or byte counts**. The original "hash cost ≪ 1% of tensor time, satisfied at every candidate b" claim was derived from counts and was falsified in the first implementation: the per-element expansion XOF alone measured 62.9% and the int-ALU mod-q combine 35.5% of per-nonce wall-time (5090, n = 4096). The XOF is fixed (§A.2 wide counter mode); the combine's limb-tensor mapping is now pinned and reference-implemented on CPU (Appendix C-13: 4 balanced base-2⁷ limbs, 16 s8×s8→s32 limb-pair GEMMs — stacked across the nonce window under §K.2b — plus an O(m²) mod-q fold; byte-identical to the direct combine). Until the C-13 kernels land in the GPU backends AND the §K.2b stage split is measured on real silicon, the digest pipeline is not yet demonstrated to meet this invariant on GPU backends.
...
| **Operand expansion (XOF, O(n²))** | <1% **only with the wide counter-mode XOF (§A.2)** — the per-element-hash variant measured **62.9%** of per-nonce wall-time on a 5090 at n=4096 (PR #89 review; fixed) | Bounded by Amdahl only if the XOF is wide; must be re-measured, not assumed. Current GPU backends expand on the **host CPU**, so this cost is real for miners until on-die generation lands. |
...
10. **Sketch tile b — REVISED AGAIN: 16 → 8 (§K.2a roofline) → 4 (§K.2b batched-sketch, PR #89 measurement).** The b=8 choice was falsified on real hardware (H100/5090 = 0.40× at n=8192); b=4 + template-scoped A/U/V + cross-nonce batching is the v4.1 profile. Confirmation items move to the §K.2b GO/NO-GO: (a) bench the b=4 verify path (8 MiB payload SHA + R=3 Freivalds) on reference CPUs against the budget; (b) measure the batched marginal stage split on real H100/B200 with `matmul_v4_stage_bench` boundaries; (c) confirm the resulting nonce/s ordering. No further roofline-only justification is acceptable for this item.
...
12. **Operand-expansion XOF — RESOLVED (wide SHA-256 counter mode; PR #89 review fix).** The first implementation violated this item's intent by hashing **once per element** (one SHA-256, keep 1 of 32 bytes), which made expansion 62.9% of per-nonce wall-time on a 5090 at n=4096 — the miner pays it **per nonce**, not once like the verifier. Now pinned: keystream block `j = SHA256(seed_le ‖ domain ‖ LE64(j))`, all 32 bytes rejection-sampled in stream order (~31.4 elements/compression, ~32× fewer hashes; `src/matmul/int8_field.cpp::ExpandBalancedS8Stream`). SHA-256 chosen over the ChaCha8/12-class recommendation to keep the consensus hash set PQ-conservative and single-primitive; the width, not the primitive, was the load-bearing property. The 15–35 ms single-thread regeneration envelope (verify budget, §I.5) still applies **and additionally** the miner-side per-nonce hash share must satisfy the §K.2a wall-time dominance check.
13. **Mod-q combine on tensor cores — CPU REFERENCE LANDED (limb-tensor path); GPU kernels + measurement OPEN.** Pinned construction (v4.1): decompose each exact-int32 entry of P = U·A and Q = B·V into **4 balanced base-2⁷ digits in [-64, 63]** (total and unique for |x| < 2²⁷, i.e. `15,625·n < 2²⁷` ⇔ **n ≤ 8589** — covering the whole 4096..8192 window; `CheckCombineLimbBound`), compute the 16 limb-pair products as native s8×s8→s32 tensor GEMMs (each exact: n·64² < 2³¹), then one shifted mod-q fold `Ĉ = Σᵢⱼ 2^{7(i+j)}·S_ij mod q` in O(m²) on int ALU. Byte-identical to the direct mod-q combine (`ComputeCombineLimbTensor` / `ComputeCombineLimbTensorStacked`, pinned by `matmul_v4_batch_tests`). Under §K.2b batching the 16 GEMMs take the stacked m × Q·m × n shape. REMAINING: implement in the CUDA/HIP/Metal backends and measure the §K.2a-WT/§K.2b stage split on real silicon.
14. **ρ launch disclosure (monitoring, never a parameter)** — re-measure consumer/datacenter AI-rental centrals at activation and recompute the price-robust floor ratio ρ (§S.4.6) for the launch disclosure; refresh quarterly with timestamps.
15. **I1′ anti-amortization relaxation — EXTERNAL ADVERSARIAL REVIEW REQUIRED (mainnet blocker; §C I1′, ACTIVATION B4).** v4.1 deliberately makes A/U/V template-scoped so `U·A` amortizes and combines batch (§K.2b). The soundness/no-pre-mining/symmetry/marginal-difficulty argument is written in §C I1′, but the marginal-work floor ("no evaluation of `M·Bᵢ·V` below reading Bᵢ + one rank-m projection") is an assumption, not a theorem, and template-scoped projectors reopen exactly the channel v4.0's I7 closed. Solicit review specifically targeting: cross-nonce shared structure in the XOF-derived Bᵢ, batch-algebra shortcuts over fixed (M, V), and difficulty-calibration gaming between template refreshes.
16. **v4.1 batched profile real-GPU measurement (§K.2b GO/NO-GO; ACTIVATION B2g)** — run `matmul_v4_stage_bench`-boundary instrumentation on physical H100/B200 (and a 5090 for the consumer anchor) at n = 4096, b = 4, Q ≥ 32; the datacenter-favoring claim remains a hypothesis until then (two prior model-based estimates were falsified).
