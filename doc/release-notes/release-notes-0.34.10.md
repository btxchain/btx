...
| Missing-frontier GETDATA | While the unique frontier is HEADER_ONLY, GETDATA is 1-wide and the download window clamps to that height **or the honest HEADER_ONLY tip-child, whichever is higher**. A competing tower with `FollowedChainAhead==0` used to 16-wide unconnectable descendants so the LCA+1 body never arrived and CPU ExactReplay confirmation never ran. |
...
| CPU mismatch confirmation | Unresolved device digest mismatches confirm on a bounded background CPU queue (`b-mmconfirm`, cap 4 pending / 64 results) instead of holding the GPU lease and the sole validation worker. Default `-matmulrcconfirmcpu=1`. Opt-out `=0` only authorizes a completed qualified exact device mismatch. ExactReplay itself stays **strict-device GPU**. A CPU-pending unique competing frontier must not re-occupy the RC slot with a Lookup-only retry: that starved honest tip-child GPU ExactReplay for the confirmation duration. |
...
| Honest HEADER_ONLY stall escape | `m_best_header_extending_tip` tracks the heaviest header that still extends the connected tip. GETDATA asks for that HEADER_ONLY tip-child from every peer (including competing-tower advertisers) and does not fill inflight / ExactReplay with competing descendants. Newly mined blocks push the next honest body to behind peers so NAT miners do not wait for a human `addnode`. RCADMIT tickets for honest tip+N remain admissible while the HEADER_ONLY tip-child is still unauthenticated, so a suffix body hits the pending-cap retain path instead of being HEADER_ONLY-dropped as competing. |
| CPU confirmation queue lifetime | `GetRCCpuConfirmationQueue()` is a process-lifetime object. `Stop()` permanently closes admission. Tests reopen it with `ResetForTest()` after callers are quiescent. The getter never deletes a published queue. |
| Persisted tip-child vs HEADER_ONLY frontier | A missing-body or CPU-pending competing frontier does not suppress `RetryMatMulDeferredBodies` / AcceptBlock re-entry of an on-disk HAVE_DATA active-tip child. |
| Per-tower acquisition GETDATA | The missing-frontier download-window clamp applies only to peers whose BestKnown contains that frontier. A withheld root on tower A cannot starve GETDATA of another registered heavier tower. |
