...
acquisition-escape ExactReplay admits only the unique parent-connectable
frontier; the followed tip-child yields while that frontier exists;
`-acquisitionstallseconds` is ignored on mainnet. See
[release-notes-0.34.10.md](release-notes/release-notes-0.34.10.md).

**0.34.9** remains the previous shipping client (`IS_RELEASE=true` on tag **v0.34.9**):
...
**0.34.8** remains the previous line (first-run, JIT capability, HCP,
Cognitive Reserve). See
