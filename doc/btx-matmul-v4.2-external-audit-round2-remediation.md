...
| ID | Audit item | Disposition |
|----|-----------|-------------|
| **P1** | Consensus block 24 MB > P2P message 16 MB; a 16–24 MB block is un-relayable | **FIXED (`12febe7`)** — the LIVE bug is fixed by raising `MAX_PROTOCOL_MESSAGE_LENGTH` 16 → 24 MB so the P2P message ceiling accommodates a maximum serialized block, with a `static_assert(MAX_BLOCK_SERIALIZED_SIZE <= MAX_PROTOCOL_MESSAGE_LENGTH)` in `net.cpp` so they can never silently diverge. The 24 MB block size is a deliberate SMILE-v2 shielded-throughput parameter, so capping the block (regressing shielded capacity chain-wide) was rejected in favour of the non-consensus P2P-limit raise; a node must be able to receive a valid 24 MB block regardless. |
| **P2** | `nMatMulProofPruneDepth` has no consumer | **DOCUMENTED** — marked NON-FUNCTIONAL/RESERVED in `consensus/params.h` with the true storage cost. Under the 8 MiB profile at 90 s spacing the growth is the same order; proof-aware pruning (reorg/reindex/assumeUTXO/RPC semantics) or removing the field is the real fix. |
| **P3** | Compact-block relay does not carry the mandatory proof (full-block fallback) | **DECIDED (proof stays IN-BLOCK)** — a fetched-sidecar / separately-retrieved mandatory proof is REJECTED: it would let a miner publish a header while withholding the proof, so a node could neither validate nor disprove it — a withholding/eclipse vector that does not exist today. The proof is kept inside the block. An optional **streaming** relay (send header+txs for instant validation, then stream the ~8 MiB proof) is an acceptable *latency* optimization for orphan rate but is NOT required and adds no data-availability risk (the proof still arrives as part of the block, not a separately-withholdable object). |

## Lead design decisions (2026-07-16, on the external transport/encoding review)
- **Encoding: stay on the 8 MiB sketch (ENC-BMX4C, m=1024, b=4).** The 32 MiB "deeper-commit" ENC-BMX4C-D variant (m=2048, b=2) is **PARKED — not the deployment path**: it quadruples the payload for marginal ordering gain and adds a data-availability cost, while the 8 MiB profile already excludes older cards and captures the main datacenter lever (the INT8 combine is ~80% of the per-nonce work, so INT8 TOPS throughput with a tuned combine kernel is the deciding factor). ENC-BMX4C-D remains in-tree but activation-disabled and is explicitly not a candidate. **Forward compute-bound work targets a tuned INT8 combine kernel on the 8 MiB profile, not a larger payload.**
- **Proof stays in-block; no sidecar** (P3 above).
- **Dimension guard retained:** the `nMatMulV4Dimension / kTileB == BMX4C_SKETCH_RANK_M (1024)` construction pin stays — it fails LOUD at startup if a dimension retarget is not matched by a lockstep tile change, preventing silent mutation of the committed object.
