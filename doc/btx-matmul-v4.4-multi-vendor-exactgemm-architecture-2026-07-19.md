...
4. **Device-resident LT path** (`matmul_v4_lt_accel.cu`): qualified IMMA runs `G·W`, radix-lowered `Y·H`, `U·Ahat`, dense-INT8 `Bhat·V`, and nine exact Karatsuba combine products. The full-header Q* entry now generates W and SHA256d(Chat) on-device, copies only digest/status records, and synchronizes once at the batch boundary. MX scale derivation is on-device, but Extract still lowers to dense INT8; no native MXFP4 instruction is issued. Scalar CUDA graphs remain the non-IMMA path.
