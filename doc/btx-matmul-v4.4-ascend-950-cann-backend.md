...
> **MatMul v4.7 reuse boundary.** This LT backend is not presently evidence
> that Ascend can validate Epoch-A Profile 1. Before it is advertised for
> v4.7 ExactReplay it must match the portable Profile-1 corpus, run the
> corrected continuous load campaign with device-coverage telemetry, and fail
> closed on mismatch. Profile 2 remains an Epoch-D proof-authority workload.
