...
| Bounty create → find → on-chain complete | `python3 test/functional/feature_modelnet_bounty_lifecycle.py --descriptors --configfile=build-gcc13/test/config.ini --timeout-factor=1` | Owned helper, unix-only bind. Publish, `searchbounties`, fund two-leaf CLTV+refund escrow, mine to locktime, `preparebountyaward`/`preparebountyrefund` spend. No auto-spend. |
