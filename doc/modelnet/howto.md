...
| Bounty create → find → complete | `python3 test/functional/feature_modelnet_bounty_lifecycle.py --descriptors --configfile=build-gcc13/test/config.ini --timeout-factor=1` | Owned helper, unix-only bind. Publish, `searchbounties`, real funding txid:vout, EXACT_CHECKS, propose/approve. No auto-spend. |
