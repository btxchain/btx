...
Status: **HISTORICAL PR95 PRECHECK ANALYSIS.** The local ExactReplay launch
candidate is MatMul v4.7 Epoch A: Profile 1 with complete epsilon-zero
ExactReplay. The sampled carrier and FVT have no consensus authority. Profile 2
is not an ExactReplay launch candidate; it is reserved for Epoch D after a
durable succinct proof has become authoritative on Profile 1. Epoch B first
requires that durable proof **and** replay; Epoch C retains Profile 1 under
proof authority. See `doc/btx-matmul-v4.7-transition-roadmap.md`. FVT's flag
and single-round primitive are retained for compatibility and analysis, but
the separate FVT gate is redundant and is not on the consensus-success path.
No activation heights, digest, wire format, carrier version, or int64 reference
changed.
...
- At the PR95 snapshot analyzed here, Profile-2 acceptance ran **only** the sampled carrier check
...
b_seq=87552, n_ctx=786432; current sampled verify ≈ 700 ms @ 32 threads)
...
## 6. Historical hypothetical re-pin impact

This section records the superseded PR95 proposal. It must not be implemented
as a direct Profile-2 activation. Under MatMul v4.7, every height remains
disabled and Profile 2 may activate only at Epoch D with authoritative durable
proofs.
...
- **Consensus constants / flags added:** `nMatMulRCProfile2FullyVerifyTerminalRound`
  (historical proposal: true at activation); optionally
  `kRCFreivaldsInteriorSampleCount` (interior λ, e.g. 512→256). These are
  deterrence settings only and cannot gate MatMul v4.7 consensus.
...
## 8. Historical implementation record (FVT)

This section records the earlier PR95 FVT implementation and is not a
description of the current local authority dispatch. FVT (§4) was wired into
consensus code as described below.
...
the candidate mainnet Epoch-A activation is Profile 1) and the coupled heights remain
`INT32_MAX`.
