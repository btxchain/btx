...
- `/path/to/btx-source/.btx-validation/m15-centos-container-single-node-artifact.json`
...
Added script:
- `/path/to/btx-source/scripts/live_regtest_realworld_validation.py`
...
- Production readiness remains contingent on unresolved design-level cryptographic hardening items tracked in:
  - `/path/to/btx-source/doc/btx-shielded-pool-critical-issues.md`
  - `/path/to/btx-source/doc/btx-design-assessment.md`
...
### New Script
Added:
- `/path/to/btx-source/scripts/live_regtest_load_stress.py`
...
1) **Rebuild**
```bash
cmake --build /path/to/btx-source/build-btx -j8
```
- result: pass

2) **Full CTest**
```bash
ctest --test-dir /path/to/btx-source/build-btx -j8 --output-on-failure
```
- result: `204/204` passed
...
```bash
python3 /path/to/btx-source/test/functional/test_runner.py \
  --configfile /path/to/btx-source/build-btx/test/config.ini \
  --jobs=6 \
  feature_btx_dgw_convergence.py feature_btx_fast_mining_phase.py mining_mainnet.py mining_matmul_basic.py \
  wallet_shielded_sendmany_stress.py wallet_shielded_mixed_stress.py wallet_shielded_topology_sim.py wallet_shielded_longhaul_sim.py \
...
```bash
scripts/live_regtest_realworld_validation.py \
  --build-dir /path/to/btx-source/build-btx \
  --mine-blocks 4000 \
