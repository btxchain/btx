...
> **MatMul v4.7 reuse boundary.** The exactness discipline here—canonical
> integer output, self-qualification, explicit device provenance, and
> fail-closed fallback—carries forward. The LT lane shapes and planner defaults
> do not. Epochs A and B require a Profile-1 ExactReplay backend to match the
> frozen portable oracle; Epochs C and D may use these lanes for mining or
> non-consensus auditing only after the succinct verifier is authoritative.
> Profile 2 is not an Epoch-A default.
